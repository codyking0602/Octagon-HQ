-- One-time same-day replacement for the August 30, 2026 UFC Daily Double.
-- Preserve the canonical Daily Challenge runtime: a new immutable schedule identity changes the
-- hidden deterministic seed while keeping the existing rotation unchanged from August 30 onward.
do $$
declare
...
begin
  ...
  if private.daily_challenge_expected_game(v_replacement_version, v_target_day) is distinct from 'keep_4_cut_4' then
    raise exception 'replacement Daily Challenge schedule % does not preserve Keep 4 Cut 4 on %',
      v_replacement_version,
      v_target_day;
  end if;
