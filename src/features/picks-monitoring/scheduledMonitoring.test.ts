...
describe("scheduled monitoring cadence", () => {
  it("uses low frequency far away and escalates through fight week and event day", () => {
    expect(scheduledMonitoringIntervalMs(event("2026-08-20T12:00:01Z"), now)).toBe(24 * 60 * 60 * 1000);
    expect(scheduledMonitoringIntervalMs(event("2026-08-15T12:00:00Z"), now)).toBe(12 * 60 * 60 * 1000);
    expect(scheduledMonitoringIntervalMs(event("2026-08-11T18:00:00Z"), now)).toBe(6 * 60 * 60 * 1000);
    expect(scheduledMonitoringIntervalMs(event("2026-08-11T00:00:00Z"), now)).toBe(2 * 60 * 60 * 1000);
    expect(scheduledMonitoringIntervalMs(event("2026-08-10T17:00:00Z"), now)).toBe(60 * 60 * 1000);
  });

  it("uses event start after Picks lock and stops after the event begins", () => {
    expect(scheduledMonitoringIntervalMs(event("2026-08-10T17:00:00Z", "2026-08-10T11:00:00Z"), now)).toBe(60 * 60 * 1000);
    expect(decideScheduledMonitoring({ event: event("2026-08-10T11:59:59Z"), now })).toEqual({ due: false, reason: "event_started" });
  });

  it("does not call again before the latest run or lease is due", () => {
    const monitored = event("2026-08-15T12:00:00Z");
  ...
  it("protects low or exhausted provider quota until a manual run observes recovery", () => {
    expect(decideScheduledMonitoring({
      event: event("2026-08-15T12:00:00Z"),
      now,
