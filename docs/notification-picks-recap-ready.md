...
Multiple unread event recaps collapse into one row, such as **Recap is ready ×2**, while the newest completed event owns the visible title and summary. After that row is read, a later completed event reopens it with a fresh count of one through the canonical notification owner.
...
Incomplete-Picks near-lock reminders, event-start timing, season-result corrections, notification preferences, and device push remain separate focused slices.
