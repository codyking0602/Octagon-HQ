...
select pg_temp.assert_true(((select count(*)::integer from private.auction_catalog where content_version='ufc-auction-2026-08-v2' and mode_id='jon-jones-performances')) is not distinct from (24), 'Jones history complete');
select pg_temp.assert_true(((select count(*)::integer from private.auction_catalog where content_version='ufc-auction-2026-08-v2' and mode_id='conor-mcgregor-performances')) is not distinct from (14), 'McGregor history complete');
select pg_temp.assert_true(((select count(*)::integer from private.auction_catalog where content_version='ufc-auction-2026-08-v2' and mode_id='charles-oliveira-performances')) is not distinct from (37), 'Oliveira history complete');
select pg_temp.assert_true(not has_table_privilege('authenticated','private.auction_catalog','select'), 'catalog remains private');
select pg_temp.assert_true((select bool_and(content_version='ufc-auction-2026-08-v2') from private.auction_catalog_versions where is_preparation_version), 'new preparations pin v2');
select pg_temp.assert_true((pg_get_functiondef('private.grade_auction(uuid)'::regprocedure)) like ('%ufc-auction-2026-08-v1%'), 'grader retains pinned v1');
select pg_temp.assert_true((pg_get_functiondef('private.grade_auction(uuid)'::regprocedure)) like ('%ufc-auction-2026-08-v2%'), 'grader accepts pinned v2');
select pg_temp.assert_true(((select (grading_inputs->>'overall')::integer from private.auction_catalog where content_version='ufc-auction-2026-08-v1' and mode_id='ultimate-fighter' and display_label='Jon Jones')) is not distinct from ((select (grading_inputs->>'overall')::integer from private.auction_catalog where content_version='ufc-auction-2026-08-v2' and mode_id='ultimate-fighter' and display_label='Jon Jones')), 'copied item absolute value is invariant');
select pg_temp.assert_true((select bool_and(grading_inputs ?& array['Striking','Grappling','Frame','Power','Heart']) from private.auction_catalog where content_version='ufc-auction-2026-08-v2' and mode_id='ultimate-fighter'), 'all Ultimate Fighter categories exist');
select pg_temp.assert_true(not exists (select 1 from private.auction_catalog where content_version='ufc-auction-2026-08-v2' group by mode_id having count(*) <> count(distinct display_label)), 'every v2 mode has unique display labels');
select pg_temp.assert_true(not exists (
  select 1 from private.auction_catalog
  where content_version='ufc-auction-2026-08-v2'
    and translate(lower(display_label), 'áéíóúÁÉÍÓÚ', 'aeiouaeiou') like '%aldo%'
    and translate(lower(display_label), 'áéíóúÁÉÍÓÚ', 'aeiouaeiou') like '%faber%'
), 'v2 excludes the non-UFC Aldo-Faber matchup in every ordering and accent form');
...
select pg_temp.assert_true(((select count(*)::integer from private.auction_deck_entries where auction_id='00000000-0000-0000-0000-0000000000d3')) is not distinct from (8), 'canonical v2 generator returns eight items');
select pg_temp.assert_true(((select count(distinct private_item_reference)::integer from private.auction_deck_entries where auction_id='00000000-0000-0000-0000-0000000000d3')) is not distinct from (8), 'canonical v2 generator has no duplicates');
select pg_temp.assert_true((select count(*) <= 2 from private.auction_deck_entries d join private.auction_catalog c on c.content_version='ufc-auction-2026-08-v2' and c.mode_id='strikers' and c.item_reference=d.private_item_reference where d.auction_id='00000000-0000-0000-0000-0000000000d3' and c.private_generation_class='ace'), 'canonical v2 generator respects ace cap');
select pg_temp.assert_true((select count(*) <= 4 from private.auction_deck_entries d join private.auction_catalog c on c.content_version='ufc-auction-2026-08-v2' and c.mode_id='strikers' and c.item_reference=d.private_item_reference where d.auction_id='00000000-0000-0000-0000-0000000000d3' and c.rarity_band>=4), 'canonical v2 generator respects high-end cap');
...
select pg_temp.assert_true(((select challenger_final_score from private.auction_games where id='00000000-0000-0000-0000-0000000000d4')) is not distinct from ((select challenger_final_score from private.auction_games where id='00000000-0000-0000-0000-0000000000d5')), 'identical v1/v2 inputs have identical contribution');
select pg_temp.assert_true(((select challenger_final_score from private.auction_games where id='00000000-0000-0000-0000-0000000000d5')) is not distinct from ((select round(avg((grading_inputs->>'overall')::numeric),2) from private.auction_catalog where content_version='ufc-auction-2026-08-v2' and mode_id='strikers' and item_reference in ('strikers-1','strikers-2','strikers-3','strikers-4'))), 'standard v2 grade is arithmetic mean of four');
select pg_temp.assert_true(((select lifecycle_state from private.auction_games where id='00000000-0000-0000-0000-0000000000d4')) is not distinct from ('completed'), 'existing pinned v1 game remains supported');
...
select pg_temp.assert_true(((select count(*)::integer from private.auction_deck_entries where auction_id='00000000-0000-0000-0000-0000000000d6')) is not distinct from (10), 'canonical Ultimate Fighter generator returns ten items');
select pg_temp.assert_true(((select count(distinct private_item_reference)::integer from private.auction_deck_entries where auction_id='00000000-0000-0000-0000-0000000000d6')) is not distinct from (10), 'canonical Ultimate Fighter generator has no duplicates');
select pg_temp.assert_true((select count(*) <= 2 from private.auction_deck_entries d join private.auction_catalog c on c.content_version='ufc-auction-2026-08-v2' and c.mode_id='ultimate-fighter' and c.item_reference=d.private_item_reference where d.auction_id='00000000-0000-0000-0000-0000000000d6' and c.private_generation_class in ('mythic','crown')), 'canonical Ultimate Fighter generator respects crown cap');
