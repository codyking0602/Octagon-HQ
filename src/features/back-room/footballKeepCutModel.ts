...
import {
  footballComparisonCategoryQuery,
...
/** Keep/Cut preserves its public query helpers while the shared comparison authority owns the criteria. */
export function footballKeepCutCategoryQuery(packId: FootballKeepCutPackId): FootballSubjectQuery {
  return footballComparisonCategoryQuery(packId);
}
...
function keepCutItemsFromCanonicalLedger(packId: FootballKeepCutPackId) {
  ...
  return footballComparisonItemsFromCanonicalLedger(packId, ratedPack.items, BOARD_SIZE);
}
