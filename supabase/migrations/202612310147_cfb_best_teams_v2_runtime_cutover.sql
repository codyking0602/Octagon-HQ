...
-- Preserve the exact pre-v2 materializer so historical/pre-cutover weeks can still
-- be reproduced from the legacy authority without ever mixing in v2 grades.
do $clone_legacy_weekly_materializer$
declare
  v_definition text;
  v_legacy_definition text;
begin
  v_definition := pg_get_functiondef('private.materialize_football_weekly_auction_week(date)'::regprocedure);
  v_legacy_definition := replace(
    v_definition,
    'private.materialize_football_weekly_auction_week(p_week_start date)',
    'private.materialize_football_weekly_auction_week_legacy(p_week_start date)'
  );
  if v_legacy_definition = v_definition then
    raise exception 'Legacy Weekly Auction materializer clone drifted';
  end if;
  execute v_legacy_definition;
end
$clone_legacy_weekly_materializer$;
revoke all on function private.materialize_football_weekly_auction_week_legacy(date) from public,anon,authenticated;
...
begin
  if p_week_start < date '2026-09-22' then
    perform private.materialize_football_weekly_auction_week_legacy(p_week_start);
    return;
  end if;
