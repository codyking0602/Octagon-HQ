...
export function createIdentityGateway(): IdentityGateway | null {
  ...
  return {
    ...
    async loadProfile(userId) {
      const { data, error } = await client.rpc("get_my_identity_profile").maybeSingle();

      if (error) throw new Error(error.message);
      if (!data) return null;
      const parsed = profileRowSchema.parse(data);
      if (parsed.id !== userId) throw new Error("Octagon HQ received a profile for the wrong account.");
      return {
        id: parsed.id,
