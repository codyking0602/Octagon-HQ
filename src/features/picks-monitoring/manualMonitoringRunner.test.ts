...
describe("provider selection and deterministic evidence", () => {
  it("filters unrelated MMA events and recomputes selected-card coverage", () => { const unrelated = { ...fixture[0], id: "unrelated", commence_time: "2026-08-20T22:00:00Z", home_team: "Other One", away_team: "Other Two", bookmakers: [] }; const filtered = filterOddsToMonitoredEvent(odds([...fixture, unrelated]), event); expect(filtered.snapshots).toHaveLength(1); expect(filtered.coverage).toEqual({ providerEvents: 1, completeSnapshots: 1, missingSnapshots: 0 }); });
