...
**Status:** Product architecture locked; Phase 0 audit complete; rollout PRs 2–8 complete through PR #815; PR 9 is in review as PR #816; PR 10 follows.  
...
- **PR 9 has one explicit execution exception:** Cody explicitly requested direct GitHub implementation and merge of PR 9 in this handoff. This does not change the default Codex routing for PR 10+.
...
| **8** | Up Next priority hero | **COMPLETE — #815; base `763a5ea69873f1869681022b2ad3a12de8d13362`; merge `42f47089f77715b2898e669aa724b22df5f49edc`; explicit ChatGPT execution exception** | **HIGH — cross-product priority logic** |
| **9** | Today's Challenges + Your HQ | **IN REVIEW — #816; base `42f47089f77715b2898e669aa724b22df5f49edc`; explicit ChatGPT execution exception** | **HIGH — reuse existing challenge / record data owners** |
| **10** | UFC HQ Home block | **CODEX REQUIRED — NEXT** | **HIGH — preserve Ranking Spotlight + Shane's Contender Series** |
...
**Status: PR 7 foundation and PR 8 Up Next hero are complete through #815; PR 9 Today's Challenges + Your HQ is in review as #816; PR 10 is next.**
...
PR #816 fills only the PR 9 Today's Challenges and Your HQ slots. UFC and Football daily products remain independent while reusing the existing sport-aware Today's Challenge runtime/repository path. Your HQ follows the locked three-stat model: Daily Streak, UFC Picks Record, and Football Picks Record. Football record data is exposed by the existing PicksProvider through its existing Picks repository owner; Home does not mount another Football PicksProvider, create a second record calculation, or add another persistence/query owner. PR #816 does not complete the PR 10 UFC HQ block or PR 11 Football HQ / What's New integration.
...
**Phase 0 is complete. PR 2 through PR 8 are complete through PR #815. PR 9 — Today's Challenges + Your HQ is in review as PR #816. PR 10 — UFC HQ Home block is the next named rollout item after PR 9 merges.**
...
- Cody explicitly requested direct GitHub execution and merge of PR 9 in the PR 9 handoff; this is a PR 9 execution exception and the default Codex routing resumes with PR 10.
- PR #816 is the PR 9 implementation, based on `main` at `42f47089f77715b2898e669aa724b22df5f49edc`.
- PR #815 / PR 8 is confirmed merged as `42f47089f77715b2898e669aa724b22df5f49edc`.
- `src/features/home/HomePage.tsx` remains the one Home composition owner and preserves the six locked section boundaries and PR 8 Up Next priority behavior.
- Today's Challenges renders UFC and Football as independent compact surfaces. Both consume the existing sport-aware Today's Challenge runtime/repository path; no new challenge provider, repository, persistence, or fallback query path is introduced.
- Your HQ follows the superseding locked three-stat model only: Daily Streak, UFC Picks Record, Football Picks Record. Favorite fighter, Open Challenges, and a forced fourth stat are not restored.
- `src/features/picks/PicksProvider.tsx` remains the Picks state owner and uses its existing repository instance to expose the canonical Football summary needed by universal Home. Home does not mount a second `PicksProvider sport="football"`, create a second record calculation, or add localStorage ownership.
- Universal Home remains neutral. UFC red and Football navy `#1F4E79` appear only inside their sport-specific challenge cards.
- PR 10 — UFC HQ Home block is next. PR 9 does not implement PR 10 UFC HQ completion or PR 11 Football HQ / What's New integration early.

### PR 8 Up Next priority hero

- Cody explicitly requested direct GitHub execution and merge of PR 8 in that chat; this is a PR 8 execution exception and the default Codex routing resumed until the separately authorized PR 9 exception.
...
- Football Picks remains owned by the existing route-scoped `PicksProvider sport="football"`. PR 8 deliberately does not mount a second Football Picks provider/query on Home; PR 11 remains responsible for canonical Football Home integration beyond the PR 9 record summary explicitly assigned to Your HQ.
