-- Allow versioned Football Home Player Spotlight media for scheduled weekly pairs.
-- Reads remain public; writes remain restricted to the existing Picks owner through the setter and storage policies.
...
alter table public.home_feature_media
  add constraint home_feature_media_key check (
    ...
    or content_key ~ '^football-player-spotlight-[a-z0-9][a-z0-9-]{1,100}-(cfb|nfl)$'
  );
...
begin
  ...
  if not (
    ...
    or v_key ~ '^football-player-spotlight-[a-z0-9][a-z0-9-]{1,100}-(cfb|nfl)$'
  ) then
