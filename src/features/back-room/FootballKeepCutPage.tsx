...
function Tray({ title, items, packId }: { title: Decision; items: FootballRankFiveItem[]; packId: FootballKeepCutRun["pack"]["id"] }) {
  return (
...
function ResultList({ title, items, packId }: { title: string; items: FootballRankFiveItem[]; packId: FootballKeepCutRun["pack"]["id"] }) {
  return (
...
function resolveChallengeRun(packId: string | null, lineupIds: readonly string[], challengeId: string): FootballKeepCutRun | null {
  ...
  try {
    ...
    return { pack, lineup, identity: footballCuratedIdentity(FOOTBALL_KEEP_CUT_GAME_ID, challengeId, lineupIds, pack.id) };
  } catch {
    return null;
  }
}
...
export default function FootballKeepCutPage() {
  ...
  const sharedChallengeId = profileMatch.challenge?.code ?? `shared:${queryPackId ?? "unknown"}:${queryLineupIds.join("|")}`;
  const sharedRun = useMemo(() => resolveChallengeRun(profilePackId, profileLineupIds, sharedChallengeId) ?? resolveChallengeRun(queryPackId, queryLineupIds, sharedChallengeId), [profilePackId, profileLineupIds.join("|"), queryPackId, queryLineupIds.join("|"), sharedChallengeId]);
  ...
  const result = useMemo(() => complete ? scoreFootballKeepCutSelection(run.lineup, kept.map((item) => item.id)) : null, [complete, kept.map((item) => item.id).join("|"), run.lineup]);
  ...
  function startNew() { reset(createRandomFootballKeepCutRun(run.pack.id)); }
  function replay() { if (shared) reset(run); else startNew(); }
  function challengeResult(nextDecisions: readonly Decision[], nextKept: readonly FootballRankFiveItem[]) {
    ...
    return { decisions: [...nextDecisions], keptIds: graded.kept.map((item) => item.id), cutIds: graded.cut.map((item) => item.id), score: graded.score, correctComparisons: graded.correctComparisons };
  }
  function decide(decision: Decision) {
    ...
    if (next.length === run.lineup.length) {
      ...
      recordLineupCompletion(run.identity, { packId: run.pack.id, ...finalResult });
      if (profileMatch.isRecipient && profileMatch.challenge?.responderResult === null) profileMatch.submitResult(asChallengeJson(finalResult));
    }
  }
  ...
  async function challengeSomeone() {
    ...
    const status = await beginChallenge({
      ...
      setup: asChallengeJson({ packId: run.pack.id, lineupIds: run.lineup.map((item) => item.id), lineup: run.lineup.map((item) => ({ id: item.id, name: item.name })) }),
      ...
      shareUrl: footballChallengeUrl("/football/keep-cut", { pack: run.pack.id, lineup: run.lineup.map((item) => item.id).join(",") }),
    });
  ...
  if (result) {
    return (
      <div className="page football-debate-page football-keep-cut-page">
        {profileMatch.creator ? <section className="challenge-game-banner"><span>PROFILE CHALLENGE</span><strong>{profileMatch.creator.displayName} sent these exact eight football subjects.</strong><small>Both Keep/Cut boards reveal after you finish.</small></section> : null}
        ...
        <GameResultActions onChallenge={() => void challengeSomeone()} onReplay={replay} onAllGames={() => navigate("/football")} replayLabel={replayLabelFor(run.identity.type)} status={challengeStatus} />
      </div>
    );
  }
  ...
  const instruction = keepFull ? "KEEP IS FULL — THIS ONE HAS TO GO" : cutFull ? "CUT IS FULL — THIS ONE HAS TO STAY" : "MAKE THE CALL. IT LOCKS IMMEDIATELY.";

  return (
    <div className="page football-debate-page football-keep-cut-page">
      {profileMatch.creator ? <section className="challenge-game-banner"><span>PROFILE CHALLENGE</span><strong>{profileMatch.creator.displayName} sent these exact eight football subjects.</strong><small>Make all eight locked calls to reveal the matchup.</small></section> : null}
      <section className="football-debate-intro">
        <div><p className="eyebrow">FOOTBALL HQ</p><h1>{run.pack.prompt}</h1><p>{run.pack.intro}</p></div>
        <div className="football-debate-category"><small>CURRENT CATEGORY</small><strong>{run.pack.name}</strong>{!shared ? <button type="button" onClick={startNew}>NEW LINEUP</button> : null}</div>
      </section>
      <section className="football-keep-cut-board">
        ...
        <div className="football-keep-cut-trays"><Tray title="keep" items={kept} packId={run.pack.id} /><Tray title="cut" items={cut} packId={run.pack.id} /></div>
        {current ? (
          <article className="football-keep-cut-current">
            <FootballSubjectVisual item={current} packId={run.pack.id} />
            <div><p className="eyebrow">{current.league} · LOCKED DECISION</p><h2>{current.name}</h2><p>{current.subtitle}</p><small className={keepFull || cutFull ? "is-forced" : ""}>{instruction}</small></div>
            <div className="football-keep-cut-current__actions"><button className="is-keep" type="button" disabled={keepFull} onClick={() => decide("keep")}>KEEP</button><button className="is-cut" type="button" disabled={cutFull} onClick={() => decide("cut")}>CUT</button></div>
          </article>
        ) : null}
      </section>
    </div>
