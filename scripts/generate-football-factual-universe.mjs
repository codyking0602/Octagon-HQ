...
const promotedOrganizations = promotedSubjects.filter((subject) => subject.kind === "franchise" || subject.kind === "program");
const promotedCoaches = promotedSubjects.filter((subject) => subject.kind === "coach");
const promotedEras = promotedSubjects.filter((subject) => subject.kind === "program-era");
const promotedGames = promotedSubjects.filter((subject) => subject.kind === "game");
...
const cfbCoachSeasons = rowObjects(readJson("data/generated/football/relationships/cfb-coach-seasons-2002-2025.json"));
...
for (const row of nflPlayers) {
  const idRows = nflPlayersById.get(String(row.sourcePlayerId)) ?? [];
  idRows.push(row); nflPlayersById.set(String(row.sourcePlayerId), idRows);
...
function nflPlayerFacts(subject) {
  ...
  if (p === "QB") {
    const attempts = sumObserved(rows, "attempts"); const completions = sumObserved(rows, "completions"); const yards = sumObserved(rows, "passingYards");
    facts.push(...compact([fact("NFL", "nfl-career-passing-completions", completions), fact("NFL", "nfl-career-passing-attempts", attempts), fact("NFL", "nfl-career-passing-yards", yards), fact("NFL", "nfl-career-passing-touchdowns", sumObserved(rows, "passingTouchdowns")), fact("NFL", "nfl-career-interceptions-thrown", sumObserved(rows, "passingInterceptions"))]));
  ...
  if (["QB", "RB", "WR", "TE"].includes(p)) {
    const carries = sumObserved(rows, "carries"); const yards = sumObserved(rows, "rushingYards");
  ...
  if (["RB", "WR", "TE"].includes(p)) facts.push(...compact([fact("NFL", "nfl-career-receptions", sumObserved(rows, "receptions")), fact("NFL", "nfl-career-targets", sumObserved(rows, "targets")), fact("NFL", "nfl-career-receiving-yards", sumObserved(rows, "receivingYards")), fact("NFL", "nfl-career-receiving-touchdowns", sumObserved(rows, "receivingTouchdowns"))]));
  if (["DL", "LB", "DB"].includes(p)) facts.push(...compact([fact("NFL", "nfl-career-solo-tackles", sumObserved(rows, "tacklesSolo")), fact("NFL", "nfl-career-tackles-for-loss", sumObserved(rows, "tacklesForLoss")), fact("NFL", "nfl-career-forced-fumbles", sumObserved(rows, "forcedFumbles")), fact("NFL", "nfl-career-sacks", sumObserved(rows, "defensiveSacks")), fact("NFL", "nfl-career-interceptions", sumObserved(rows, "defensiveInterceptions")), fact("NFL", "nfl-career-passes-defended", sumObserved(rows, "passesDefended"))]));
  if (p === "K") {
    const attempts = sumObserved(rows, "fieldGoalsAttempted"); const made = finite(attempts) && attempts > 0 ? sumKnown(rows, "fieldGoalsMade") : null;
  ...
  if (p === "P") {
    const punts = sumObserved(rows, "puntingAttempts"); const yards = sumObserved(rows, "puntingYards");
...
function cfbPlayerFacts(subject) {
  ...
  if (["DL", "LB", "DB"].includes(p)) facts.push(...compact([fact("CFB", "cfb-career-defensive-interceptions", sumObserved(rows, "defensiveInterceptions")), fact("CFB", "cfb-career-sacks", sumObserved(rows, "sacks")), fact("CFB", "cfb-career-pass-breakups", sumObserved(rows, "passBreakups")), fact("CFB", "cfb-career-forced-fumbles", sumObserved(rows, "forcedFumbles")), fact("CFB", "cfb-career-fumble-recoveries", sumObserved(rows, "fumbleRecoveries")), fact("CFB", "cfb-best-season-sacks", maxObserved(rows, "sacks")), fact("CFB", "cfb-best-season-defensive-interceptions", maxObserved(rows, "defensiveInterceptions"))]));
  if (p === "K") {
    const attempts = sumObserved(rows, "fieldGoalsAttempted"); const made = finite(attempts) && attempts > 0 ? sumKnown(rows, "fieldGoalsMade") : null;
...
function nflPlayerSeasonFacts(subject) {
  if (subject.position !== "QB" || subject.season == null) return [];
  const rows = nflRowsFor(subject);
  if (!rows.length) return [];
  const attempts = sumObserved(rows, "attempts"); const completions = sumObserved(rows, "completions"); const yards = sumObserved(rows, "passingYards"); const tds = sumObserved(rows, "passingTouchdowns"); const ints = sumObserved(rows, "passingInterceptions");
  const facts = compact([fact("NFL", "nfl-season-passing-yards", yards), fact("NFL", "nfl-season-passing-touchdowns", tds), fact("NFL", "nfl-season-interceptions", ints)]);
  if ([attempts, completions, yards, tds, ints].every(finite) && attempts > 0) {
    const a = Math.min(2.375, Math.max(0, (completions / attempts - .3) * 5));
    const b = Math.min(2.375, Math.max(0, (yards / attempts - 3) * .25));
    const c = Math.min(2.375, Math.max(0, tds / attempts * 20));
    const d = Math.min(2.375, Math.max(0, 2.375 - ints / attempts * 25));
    facts.push(derived("NFL", "nfl-season-passer-rating", (a + b + c + d) / 6 * 100, "NFL passer-rating formula from bounded season completions, attempts, yards, touchdowns, and interceptions"));
  }
  return facts;
}
...
const cfbResultsBySeasonProgramName = new Map(cfbTeamResults.map((row) => [`${row.season}:${normalized(row.programName ?? row.teamName ?? row.sourceProgramId)}`, row]));
function cfbCoachFacts(subject) {
  if (subject.league !== "CFB") return [];
  const assignments = cfbCoachSeasons.filter((row) => normalized(row.coachName) === normalized(subject.name) && withinWindow(row, subject));
  const rows = assignments.map((assignment) => cfbResultsBySeasonProgramName.get(`${assignment.season}:${normalized(assignment.programName)}`)).filter(Boolean);
  if (!rows.length) return [];
  return compact([relationshipFact("cfb-coach-career-wins", sumKnown(rows, "overallWins")), relationshipFact("cfb-coach-career-losses", sumKnown(rows, "overallLosses")), relationshipFact("cfb-coach-career-ties", sumKnown(rows, "overallTies")), relationshipFact("cfb-coach-national-titles", rows.filter((row) => row.explicitNationalChampion).length)]);
}
function cfbEraFacts(subject) {
  if (subject.league !== "CFB" || !subject.school) return [];
  const rows = cfbTeamResults.filter((row) => normalized(row.programName ?? row.teamName ?? row.sourceProgramId) === normalized(subject.school) && withinWindow(row, subject));
  if (!rows.length) return [];
  return compact([relationshipFact("cfb-era-wins", sumKnown(rows, "overallWins")), relationshipFact("cfb-era-losses", sumKnown(rows, "overallLosses")), relationshipFact("cfb-era-national-titles", rows.filter((row) => row.explicitNationalChampion).length)]);
}
...
function gameFacts(subject) {
  ...
  return compact([relationshipFact("football-game-home-score", homeScore), relationshipFact("football-game-away-score", awayScore), finite(homeScore) && finite(awayScore) ? relationshipDerived("football-game-margin", Math.abs(homeScore - awayScore), "absolute value of home score - away score") : null, subject.league === "NFL" ? relationshipFact("football-game-overtime", row.overtime ? 1 : 0) : null, subject.league === "NFL" ? relationshipFact("football-game-postseason", row.gameType !== "REG" ? 1 : 0) : relationshipFact("football-game-postseason", String(row.seasonType).toLowerCase() !== "regular" ? 1 : 0), subject.league === "NFL" ? relationshipFact("football-game-championship", row.superBowl ? 1 : 0) : (row.explicitNationalChampionshipGame ? relationshipFact("football-game-championship", 1) : null)]);
}
...
for (const subject of promotedEras) { const facts = cfbEraFacts(subject); if (facts.length) records.push({ subjectId: subject.id, scope: "cfb-program-era", facts }); }
...
const permanentKinds = [["player-season", "Player seasons"], ["team-season", "Team seasons"], ["organization", "Franchises / programs"], ["coach", "Head coaches"], ["program-era", "Eras / dynasties"], ["game", "Notable games / misc"]];
for (const league of ["NFL", "CFB"]) for (const [kind, label] of permanentKinds) {
  const universe = promotedSubjects.filter((subject) => subject.league === league && (kind === "organization" ? subject.kind === "franchise" || subject.kind === "program" : subject.kind === kind));
  const hydrated = universe.filter((subject) => recordsBySubject.has(subject.id));
  matrixRows.push({ league, pool: label, universeSubjects: universe.length, subjectsWithFacts: hydrated.length, readinessPct: universe.length ? Number((hydrated.length / universe.length * 100).toFixed(1)) : 0, averageFacts: hydrated.length ? Number((hydrated.reduce((sum, subject) => sum + recordsBySubject.get(subject.id).facts.length, 0) / hydrated.length).toFixed(2)) : 0 });
}
