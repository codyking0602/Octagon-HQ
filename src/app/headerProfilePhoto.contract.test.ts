...
describe("signed-in header profile control", () => {
  it("renders the 46px photo and username inside one continuous outlined control", () => {
    ...
    expect(identityStyles).toContain(".identity-trigger.is-ready { gap: 0; overflow: hidden; height: 46px; padding: 0; border: 0; border-radius: 15px; background: var(--surface); box-shadow: inset 0 0 0 1px rgba(229, 9, 20, .48);");
    ...
    expect(identityStyles).toContain("border-right: 1px solid rgba(229, 9, 20, .48); border-radius: 15px 0 0 15px; box-shadow: none;");
