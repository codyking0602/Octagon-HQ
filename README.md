...
- [`docs/HANDOFF.md`](docs/HANDOFF.md) — current production architecture, deployment ownership, live product state, next safe action, and canonical repository tool routing.
...
Do not revive superseded roadmap files or treat historical implementation docs as current product direction. Resolve current `main` before every branch. Repository work must follow the GitHub-first tool-routing contract in `docs/HANDOFF.md`; do not default to local clones or retry-loop searches.
