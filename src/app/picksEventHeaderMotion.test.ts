...
describe("Picks event header motion", () => {
  it("adds subtle poster motion without replacing the persisted header owner", () => {
    expect(motion).toContain(".picks-event-hero.has-poster .picks-event-hero__poster");
    expect(motion).toContain("background-size: cover, 102.5% auto, cover");
    expect(motion).toContain("picks-event-header-atmosphere");
