-- Reuse the one canonical Picks scheduler wake-up for Football final settlement.
-- No second cron owner is introduced: the existing job continues to own all scheduled Picks work.
do $$
declare
  v_job_id bigint;
begin
  ...
  $job$
    select
      ...
      net.http_post(
        ...
        body := '{"mode":"scheduled"}'::jsonb,
        timeout_milliseconds := 60000
      ) as football_settlement_request_id;
  $job$
);

-- Expose official final scores through the existing current-event owner so the
-- Football Picks UI can grade each user's frozen-line ATS selection immediately.
