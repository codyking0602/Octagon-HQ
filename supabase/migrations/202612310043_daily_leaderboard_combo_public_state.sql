-- Completed Daily Double progress is persisted as the combo wrapper, while the canonical
-- runtime result projection exposes the active Keep 4, Cut 4 child state plus the completed
-- Blind Rank result. Keep the leaderboard RPC as the sole public owner and shape that
-- persisted public state the same way before it reaches the canonical result renderer.
