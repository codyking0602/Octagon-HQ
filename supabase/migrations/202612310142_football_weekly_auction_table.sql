-- Expose resolved Weekly Auction strategy state without leaking today's sealed bids.
...
-- Contract: public strategy state may use resolved awards, but never today's private bid rows or hidden grades.
do $football_weekly_auction_table_contract$
declare
  v_definition text;
begin
  ...
  if position('private.football_weekly_auction_awards' in v_definition) = 0
    or position('private.football_weekly_auction_daily_entries' in v_definition) = 0 then
    raise exception 'Weekly Auction table lost its resolved participant/award owners';
  end if;
...
$football_weekly_auction_table_contract$;
