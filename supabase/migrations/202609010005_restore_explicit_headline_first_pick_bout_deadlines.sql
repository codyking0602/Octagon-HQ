-- Restore the explicit headline-first deadline requirement after migration
-- 202609010004 was deployed from a reopened competing PR. Keep the same one
-- calculator and Event Setup publication owner: position 1 is the headline
-- main event and receives the event deadline; every following fight receives
-- a deadline exactly 30 minutes earlier.
...
-- Repair the chronological schedule written by migration 004 through the same
-- guarded calculator used by all future Event Setup publication.
do $$
declare
  v_event_id text;
begin
