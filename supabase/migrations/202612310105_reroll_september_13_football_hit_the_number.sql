-- One-time same-day Football Daily question refresh for September 13, 2026.
-- Keep Hit the Number and the existing canonical Football Daily runtime, but change the
-- immutable schedule identity so the deterministic board is regenerated with a new question.
do $$
declare
  v_target_day constant date := date '2026-09-13';
  v_source_version constant text := 'football-daily-v5';
  v_replacement_version constant text := 'football-daily-v6-question-refresh';
...
begin
  ...
  select daily.id, daily.game_type, setup.public_setup->>'metric_id'
  into v_existing_daily_id, v_existing_game, v_existing_metric
  ...
  if v_existing_daily_id is not null then
    if v_central_today <> v_target_day then
      raise exception 'September 13 Football Daily refresh is only safe on %, current Central day is %',
        v_target_day,
        v_central_today;
    end if;
    ...
    if v_existing_metric is distinct from 'nfl-team-defensive-interceptions' then
  ...
  if not found then
    raise exception 'source Football Daily schedule % disappeared during refresh', v_source_version;
  end if;
