...
const sportSchema = z.enum(["ufc", "football"]);
...
export interface TodayChallengeStreak { currentStreak: number; bestStreak: number; }
...
type TodayChallengeClient = {
  functions: { invoke: (name: string, options: { body: Record<string, unknown> }) => PromiseLike<{ data: unknown; error: FunctionError | null }> };
...
function toAttempt(row: z.infer<typeof attemptSchema>): OfficialAttempt {
  return { nativeScore: row.native_score, normalizedScore: row.normalized_score, completedAt: row.completed_at, publicResult: row.public_result };
}
...
async function invokeRuntime(client: TodayChallengeClient, body: Record<string, unknown>) {
  ...
  const message = typeof payload?.message === "string" ? payload.message : error.message || "Octagon HQ could not sync Today’s Challenge.";
...
async function rpc(client: TodayChallengeClient, name: string, args?: Record<string, unknown>) {
  ...
  if (error) throw new TodayChallengeRepositoryError("DAILY_HISTORY_FAILED", error.message || "Octagon HQ could not sync Today’s Challenge.");
  return data;
}
...
export function createTodayChallengeRepository(
  ...
  const client = suppliedClient === undefined ? getSupabaseClient() as unknown as TodayChallengeClient | null : suppliedClient;
  ...
  return {
    ...
    async loadHistory() {
      ...
      return rows.map((row) => ({ day: row.day, scheduleVersion: row.schedule_version, gameType: row.game_type, nativeScore: row.native_score, normalizedScore: row.normalized_score, completedAt: row.completed_at, publicResult: row.public_result }));
    },
    async loadStreak() {
    ...
    async loadStandings() {
      ...
      return {
        ...
        entries: row.entries.map((entry) => ({
          ...
          gameAverages: { findLeader: entry.game_averages.find_leader, wavelength: entry.game_averages.wavelength, blindResume: entry.game_averages.blind_resume, blindRank5: entry.game_averages.blind_rank_5, keep4Cut4: entry.game_averages.keep_4_cut_4 },
          isCurrentUser: entry.is_current_user,
        })),
      };
    },
    async loadDailyLeaderboard(day, scheduleVersion) {
      const row = leaderboardSchema.parse(await rpc(client, "get_daily_challenge_leaderboard", { p_day: day, p_schedule_version: scheduleVersion }));
      return {
