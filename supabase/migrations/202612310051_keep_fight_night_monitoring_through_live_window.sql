-- Keep the existing Picks monitoring owner alive through the same bounded Fight Night
-- window already used by ESPN live-state polling. The legacy odds-monitoring boundary
-- excluded a published card as soon as its master lock/event start passed, which made
-- the canonical runner lose the event before ESPN could own fight-by-fight locking.
--
-- Do not create a second selector or provider path. Repair the existing service core
-- and owner-inbox core in place. Staged cards remain pre-event only; published active
-- cards remain monitorable until 12 hours after main-card start, matching
-- shouldPollEspnLiveFightState(). Completed cards remain excluded.
...
create or replace function private.get_pick_monitoring_inbox_actionable_core()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  -- A published active Fight Night survives its legacy deadline through the same
  -- bounded 12-hour ESPN polling tail.
  select jsonb_build_object(
    'kind', 'current',
    'event_id', event.event_id,
