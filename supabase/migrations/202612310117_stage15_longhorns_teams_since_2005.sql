-- Stage 15: Texas Longhorns Teams Since 2005.
-- Reuses the existing server-owned sealed-bid Draft Room architecture.
-- Only completed Texas seasons 2005-2025 are eligible; 2026 is intentionally excluded.
...
alter table private.auction_catalog_versions
  ...
  check (game_id in ('auction', 'draft-room', 'draft-room-trio', 'draft-room-longhorns', 'draft-room-longhorn-teams'));
...
insert into private.auction_catalog_versions (
  ...
  'football-draft-room-longhorn-teams-2005-board-2026-09-v1',
  'football-draft-room-longhorn-teams-2005-grading-2026-09-v1',
  true,
  'draft-room-longhorn-teams'
);

create table private.draft_room_longhorn_teams_pool (
  season_reference text primary key,
  season_year integer not null unique check (season_year between 2005 and 2025),
  display_label text not null unique,
  hidden_grade numeric(5,2) not null check (hidden_grade between 58 and 100),
...
insert into private.draft_room_longhorn_teams_pool (
  ...
  ('texas-2005', 2005, '2005 Texas', 100, 'A'),
  ('texas-2008', 2008, '2008 Texas', 97, 'A'),
  ('texas-2009', 2009, '2009 Texas', 95, 'A'),
  ('texas-2023', 2023, '2023 Texas', 94, 'A'),
  ('texas-2024', 2024, '2024 Texas', 92, 'A'),
  ('texas-2018', 2018, '2018 Texas', 87, 'B'),
  ('texas-2025', 2025, '2025 Texas', 86, 'B'),
  ('texas-2022', 2022, '2022 Texas', 84, 'B'),
  ('texas-2006', 2006, '2006 Texas', 83, 'B'),
  ('texas-2007', 2007, '2007 Texas', 82, 'B'),
  ('texas-2020', 2020, '2020 Texas', 81, 'C'),
  ('texas-2012', 2012, '2012 Texas', 80, 'C'),
  ('texas-2019', 2019, '2019 Texas', 79, 'C'),
  ('texas-2011', 2011, '2011 Texas', 78, 'C'),
  ('texas-2013', 2013, '2013 Texas', 74, 'C'),
  ('texas-2017', 2017, '2017 Texas', 73, 'D'),
  ('texas-2021', 2021, '2021 Texas', 66, 'D'),
  ('texas-2014', 2014, '2014 Texas', 63, 'D'),
  ('texas-2010', 2010, '2010 Texas', 61, 'E'),
  ('texas-2016', 2016, '2016 Texas', 60, 'E'),
  ('texas-2015', 2015, '2015 Texas', 58, 'E');

create table private.draft_room_longhorn_teams_board_shapes (
...
insert into private.draft_room_longhorn_teams_board_shapes (shape, roll_start, roll_end) values
  ('Wide',        0.00, 0.12),
  ('Balanced',    0.12, 0.38),
  ('TopHeavy',    0.38, 0.54),
  ('BottomHeavy', 0.54, 0.70),
  ('Compressed',  0.70, 0.90),
  ('Chaotic',     0.90, 1.00);

create table private.draft_room_longhorn_teams_board_variants (
  shape text not null references private.draft_room_longhorn_teams_board_shapes(shape),
...
insert into private.draft_room_longhorn_teams_board_variants (shape, variant, quality_bands) values
  ...
  ('BottomHeavy', 3, array['A','C','C','C','D','E','E','E']),
  ('BottomHeavy', 4, array['B','C','C','C','D','D','D','E']),
  ...
  ('Compressed', 3, array['C','C','C','C','C','D','D','D']),
  ('Compressed', 4, array['C','C','D','D','D','E','E','E']),
...
create or replace function private.draft_room_longhorn_teams_board_shape(p_roll double precision)
returns text
language plpgsql
stable
set search_path = ''
...
begin
  if p_roll is null or p_roll < 0 or p_roll >= 1 then
    raise exception 'Longhorns Teams board-shape roll must be in [0,1)';
  end if;

  select shape.shape into v_shape
  from private.draft_room_longhorn_teams_board_shapes shape
  ...
  if v_shape is null then
    raise exception 'Longhorns Teams board-shape configuration does not cover roll %', p_roll;
  end if;
  return v_shape;
end;
$$;

create or replace function private.draft_room_longhorn_teams_board_variant(p_roll double precision)
returns integer
language plpgsql
immutable
set search_path = ''
...
begin
  if p_roll is null or p_roll < 0 or p_roll >= 1 then
    raise exception 'Longhorns Teams board-variant roll must be in [0,1)';
  end if;
  return floor(p_roll * 4)::integer + 1;
end;
$$;

create or replace function private.draft_room_longhorn_teams_quality_band(
  p_shape text,
  p_variant integer,
  p_slot integer
)
...
begin
  if p_slot not between 1 and 8 then
    raise exception 'Longhorns Teams board slot must be between 1 and 8';
  end if;

  select variant.quality_bands into v_bands
  from private.draft_room_longhorn_teams_board_variants variant
  where variant.shape = p_shape
    and variant.variant = p_variant;

  if v_bands is null then
    raise exception 'Unknown Longhorns Teams board variant: shape=%, variant=%', p_shape, p_variant;
  end if;
...
create table private.draft_room_longhorn_teams_board_entries (
  ...
  season_reference text not null references private.draft_room_longhorn_teams_pool(season_reference),
  season_year integer not null check (season_year between 2005 and 2025),
...
create or replace function private.protect_draft_room_longhorn_teams_calibration()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  raise exception 'Longhorns Teams Since 2005 calibration records are immutable';
end;
$$;

create trigger draft_room_longhorn_teams_pool_immutable
before update or delete on private.draft_room_longhorn_teams_pool
for each row execute function private.protect_draft_room_longhorn_teams_calibration();

create trigger draft_room_longhorn_teams_shapes_immutable
before update or delete on private.draft_room_longhorn_teams_board_shapes
for each row execute function private.protect_draft_room_longhorn_teams_calibration();

create trigger draft_room_longhorn_teams_variants_immutable
before update or delete on private.draft_room_longhorn_teams_board_variants
for each row execute function private.protect_draft_room_longhorn_teams_calibration();

create or replace function private.protect_draft_room_longhorn_teams_board_entry()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  ...
  raise exception 'Longhorns Teams Since 2005 board entries are immutable';
end;
$$;

create trigger draft_room_longhorn_teams_board_entries_immutable
before update or delete on private.draft_room_longhorn_teams_board_entries
for each row execute function private.protect_draft_room_longhorn_teams_board_entry();

revoke all on private.draft_room_longhorn_teams_pool from public, anon, authenticated;
revoke all on private.draft_room_longhorn_teams_board_shapes from public, anon, authenticated;
revoke all on private.draft_room_longhorn_teams_board_variants from public, anon, authenticated;
revoke all on private.draft_room_longhorn_teams_board_entries from public, anon, authenticated;
revoke all on function private.draft_room_longhorn_teams_board_shape(double precision) from public, anon, authenticated;
revoke all on function private.draft_room_longhorn_teams_board_variant(double precision) from public, anon, authenticated;
revoke all on function private.draft_room_longhorn_teams_quality_band(text,integer,integer) from public, anon, authenticated;
revoke all on function private.protect_draft_room_longhorn_teams_calibration() from public, anon, authenticated;
revoke all on function private.protect_draft_room_longhorn_teams_board_entry() from public, anon, authenticated;

create or replace function private.generate_draft_room_longhorn_teams_deck(p_auction_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_index integer;
  v_band text;
  v_season private.draft_room_longhorn_teams_pool;
  v_refs text[];
...
begin
  ...
  if v_game.id is null or v_game.mode_id <> 'longhorns-teams-2005' then
    raise exception 'Longhorns Teams deck generation requires a matching Draft Room game';
  end if;

  if exists (
    select 1 from private.auction_deck_entries deck where deck.auction_id = p_auction_id
  ) or exists (
    select 1 from private.draft_room_longhorn_teams_board_entries entry where entry.auction_id = p_auction_id
  ) then
    raise exception 'Longhorns Teams Since 2005 deck is already fixed';
  end if;

  v_shape := private.draft_room_longhorn_teams_board_shape(random());
  v_variant := private.draft_room_longhorn_teams_board_variant(random());

  for v_attempt in 1..12 loop
    v_refs := array[]::text[];

    for v_slot in 1..8 loop
      v_band := private.draft_room_longhorn_teams_quality_band(v_shape, v_variant, v_slot);

      select season.* into v_season
      from private.draft_room_longhorn_teams_pool season
      ...
      if v_season.season_reference is null then
        raise exception 'Longhorns Teams board generation underfilled quality band %', v_band;
      end if;
    ...
    v_elite_trio :=
      'texas-2005' = any(v_refs)
      and 'texas-2008' = any(v_refs)
      and 'texas-2009' = any(v_refs);
    ...
    with recent as (
      ...
      from private.auction_games auction
      where auction.mode_id = 'longhorns-teams-2005'
    ...
    board_overlaps as (
      ...
      from recent
      join private.draft_room_longhorn_teams_board_entries entry
        on entry.auction_id = recent.id
  ...
  if array_length(v_refs, 1) <> 8 or array_length(v_slot_order, 1) <> 8 then
    raise exception 'Longhorns Teams board shuffle failed';
  end if;
  ...
  for v_position in 1..8 loop
    ...
    v_item_reference := 'longhorn-team-' || v_position::text;

    insert into private.draft_room_longhorn_teams_board_entries (
      auction_id,
      deck_position,
      item_reference,
      board_shape,
      ...
      v_variant,
      v_index,
      v_season.season_reference,
      v_season.season_year,
      v_season.display_label,
      v_season.quality_band
    );
  ...
  if (
    select count(*) from private.draft_room_longhorn_teams_board_entries entry
    where entry.auction_id = p_auction_id
  ) <> 8
    or (
      select count(distinct entry.season_reference)
      from private.draft_room_longhorn_teams_board_entries entry
      where entry.auction_id = p_auction_id
    ) <> 8
    or (
  ...
  then
    raise exception 'Longhorns Teams board generation drifted from the eight-season contract';
  end if;
end;
$$;

revoke all on function private.generate_draft_room_longhorn_teams_deck(uuid) from public, anon, authenticated;

create or replace function private.grade_draft_room_longhorn_teams(p_auction_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if v_game.id is null
    or v_game.mode_id <> 'longhorns-teams-2005'
    or v_game.grading_version <> 'football-draft-room-longhorn-teams-2005-grading-2026-09-v1'
  then
    raise exception 'Longhorns Teams Since 2005 grading boundary is invalid';
  end if;
  ...
  join private.draft_room_longhorn_teams_pool season
    on season.season_reference = entry.season_reference
  ...
  then
    raise exception 'Longhorns Teams grading inputs are incomplete or invalid';
  end if;
...
revoke all on function private.grade_draft_room_longhorn_teams(uuid) from public, anon, authenticated;
...
alter table private.auction_games
  ...
  add constraint auction_games_round_valid check (
    ...
    and current_round <= case
      ...
      when mode_id in ('longhorns-2005', 'longhorns-teams-2005') then 8
  ...
  add constraint auction_games_selection_counts_valid check (
    challenger_selection_count between 0 and case
      ...
      when mode_id in ('longhorns-2005', 'longhorns-teams-2005') then 4
  ...
  add constraint auction_games_bankroll_ceiling check (
    challenger_bankroll <= case
      ...
      when mode_id in ('longhorns-2005', 'longhorns-teams-2005') then 40
...
begin
  ...
  v_next := replace(
    ...
    E'begin\n  if p_mode_id = ''longhorns-teams-2005'' then\n    if p_count <> 8 then raise exception ''Longhorns Teams Since 2005 deck must contain eight seasons''; end if;\n    if p_random_order is not null then raise exception ''Injected Auction random order is unavailable for Longhorns Teams rooms''; end if;\n    perform private.generate_draft_room_longhorn_teams_deck(p_auction_id);\n    return;\n  end if;\n\n  if p_mode_id = ''longhorns-2005'' then'
  );
  if v_next = v_definition then raise exception 'Longhorns Teams generator contract drifted'; end if;
  execute v_next;
  ...
  v_next := replace(
    ...
    E'  if v_game.mode_id = ''longhorns-teams-2005''\n    and v_game.grading_version = ''football-draft-room-longhorn-teams-2005-grading-2026-09-v1''\n  then\n    perform private.grade_draft_room_longhorn_teams(p_auction_id);\n    return;\n  end if;\n\n  if v_game.mode_id = ''longhorns-2005'''
  );
  if v_next = v_definition then raise exception 'Longhorns Teams grading contract drifted'; end if;
  execute v_next;
  ...
  v_next := replace(
    v_next,
    E'(''build-qb'', ''build-qb-cfb'', ''trio-nfl'', ''trio-cfb'', ''longhorns-2005'')',
    E'(''build-qb'', ''build-qb-cfb'', ''trio-nfl'', ''trio-cfb'', ''longhorns-2005'', ''longhorns-teams-2005'')'
  );
  v_next := replace(
    v_next,
    E'when p_mode_id = ''longhorns-2005'' then 8',
    E'when p_mode_id in (''longhorns-2005'', ''longhorns-teams-2005'') then 8'
  );
  v_next := replace(
    v_next,
    E'when p_mode_id = ''longhorns-2005'' then 40',
    E'when p_mode_id in (''longhorns-2005'', ''longhorns-teams-2005'') then 40'
  );
  if v_next = v_definition then raise exception 'Longhorns Teams preparation contract drifted'; end if;
  execute v_next;
  ...
  v_next := replace(
    v_next,
    E'    when v_game.mode_id = ''longhorns-2005'' then ''Longhorns Since 2005''',
    E'    when v_game.mode_id = ''longhorns-2005'' then ''Longhorns Since 2005''\n    when v_game.mode_id = ''longhorns-teams-2005'' then ''Longhorns Teams Since 2005'''
  );
  v_next := replace(
    v_next,
    E'when v_game.mode_id = ''longhorns-2005'' then v_creator_name || '' challenged you to Longhorns Since 2005.''',
    E'when v_game.mode_id = ''longhorns-2005'' then v_creator_name || '' challenged you to Longhorns Since 2005.''\n    when v_game.mode_id = ''longhorns-teams-2005'' then v_creator_name || '' challenged you to Longhorns Teams Since 2005.'''
  );
  if v_next = v_definition then raise exception 'Longhorns Teams first-bid notification contract drifted'; end if;
  execute v_next;
  ...
  if v_next = v_definition then raise exception 'Longhorns Teams submit-bid contract drifted'; end if;
  execute v_next;
  ...
  if v_next = v_definition then raise exception 'Longhorns Teams decline-sync contract drifted'; end if;
  execute v_next;
  ...
  v_next := replace(
    v_next,
    'catalog.item_reference is not null or trio.item_reference is not null or longhorn.item_reference is not null',
  ...
  if v_next = v_definition then raise exception 'Longhorns Teams participant projection contract drifted'; end if;
  execute v_next;
end;
$$;
...
begin
  if (select count(*) from private.draft_room_longhorn_teams_pool) <> 21 then
    raise exception 'Longhorns Teams Since 2005 must contain exactly 21 completed seasons';
  end if;
  ...
  if exists (
    ...
    raise exception 'Incomplete 2026 season must not enter Longhorns Teams Since 2005';
  end if;

  if (select count(*) from private.draft_room_longhorn_teams_board_shapes) <> 6
    or (select count(*) from private.draft_room_longhorn_teams_board_variants) <> 24
  then
    raise exception 'Longhorns Teams board calibration must define six shapes and 24 variants';
  end if;

  if (select sum(roll_end - roll_start) from private.draft_room_longhorn_teams_board_shapes) <> 1 then
    raise exception 'Longhorns Teams board-shape weights must cover exactly 100 percent';
  end if;
end;
$$;
