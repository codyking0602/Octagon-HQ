...
begin
  ...
  select p.id, c.pin_hash, c.failed_attempts, c.locked_until
    into v_profile_id, v_pin_hash, v_failed_attempts, v_locked_until
  ...
  if not found then
    ...
    return query select null::uuid, 'invalid'::text, 0;
    return;
  end if;
  ...
  if p_pin !~ '^\d{4}$' or v_pin_hash <> extensions.crypt(p_pin, v_pin_hash) then
    ...
    if v_next_locked_until is not null then
      return query select v_profile_id, 'locked'::text, 300;
    else
      return query select v_profile_id, 'invalid'::text, 0;
    end if;
    return;
  end if;
  ...
  return query select v_profile_id, 'ok'::text, 0;
end;
$$;
