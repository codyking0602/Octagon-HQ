...
export function ChallengeCenter({ sport = "ufc" }: { sport?: PlaySport }) {
  ...
  useEffect(() => {
    ...
    const requested = sportChallenges.find((challenge) => challenge.code === requestedCode);
    if (!requested) return;
  ...
  }, [activeProfile, loading, markOpened, navigate, requestedCode, sportChallenges, viewResults]);
  ...
  if (!enabled || !activeProfile) {
    return (
      ...
      >
        ...
        <p className="challenge-center__hint">Sign in to send and receive the same locked {sport === "football" ? "Football" : "UFC"} games across devices.</p>
