...
import { isNflWhoAmIBatch1Subject, isNflWhoAmIBatch2Subject, isNflWhoAmIBatch3Subject } from "./footballWhoAmICuration";
  ...
  it("feeds completed canonical person-identity knowledge into the actual UFC and football clue candidates", () => {
    ...
    for (const league of ["NFL", "CFB"] as const) {
      ...
      for (const candidate of universe.candidates) {
        ...
        covered += 1;
        const identityClues = candidate.clues.filter((clue) => clue.identityKnowledge);
        if (league === "NFL" && (isNflWhoAmIBatch1Subject(candidate.id) || isNflWhoAmIBatch2Subject(candidate.id) || isNflWhoAmIBatch3Subject(candidate.id))) {
  ...
  it("audits complete canonical UFC, NFL, and CFB populations with deterministic diverse sequences", () => {
    ...
    for (const [league, universe] of universes) {
      ...
      for (const candidate of universe.candidates) {
        ...
        const intentionallyCurated = league === "NFL" && (isNflWhoAmIBatch1Subject(candidate.id) || isNflWhoAmIBatch2Subject(candidate.id) || isNflWhoAmIBatch3Subject(candidate.id));
