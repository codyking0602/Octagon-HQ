...
begin
  ...
  if not exists (
    ...
    raise exception 'Draft Room v6 preparation version is missing';
  end if;
  ...
  if (
    ...
    raise exception 'NFL Build a QB v6 must retain all 60 audited QBs';
  end if;
  ...
  if exists (
    ...
    raise exception 'NFL Build a QB v6 grading packet must contain only the four canonical traits plus their four-trait overall';
  end if;

  insert into auth.users(id,instance_id,aud,role,email,encrypted_password,email_confirmed_at,created_at,updated_at,raw_user_meta_data)
  values
    (v_admin_a,'00000000-0000-0000-0000-000000000000','authenticated','authenticated','stage12-v6-admin-a@login.octagon-hq.app','',now(),now(),now(),jsonb_build_object('display_name','STAGE 12 V6 ADMIN A','historical_unclaimed',true)),
    (v_admin_b,'00000000-0000-0000-0000-000000000000','authenticated','authenticated','stage12-v6-admin-b@login.octagon-hq.app','',now(),now(),now(),jsonb_build_object('display_name','STAGE 12 V6 ADMIN B','historical_unclaimed',true)),
    (v_member,'00000000-0000-0000-0000-000000000000','authenticated','authenticated','stage12-v6-member@login.octagon-hq.app','',now(),now(),now(),jsonb_build_object('display_name','STAGE 12 V6 MEMBER','historical_unclaimed',true));

  perform public.register_unclaimed_pin_profile(v_admin_a,'Stage 12 V6 Admin A','SA');
  perform public.register_unclaimed_pin_profile(v_admin_b,'Stage 12 V6 Admin B','SB');
  perform public.register_unclaimed_pin_profile(v_member,'Stage 12 V6 Member','SM');
  ...
  then
    raise exception 'prepared NFL Draft Room did not pin the four-trait $40 contract: %', row_to_json(v_state);
  end if;

  if (select count(*) from private.auction_deck_entries deck where deck.auction_id = v_game) <> 8 then
    raise exception 'NFL Build a QB deck must contain exactly eight QBs';
  end if;
  ...
  begin
    ...
    raise exception 'Clutch remained a playable NFL Build a QB slot';
  exception when others then
  ...
  then
    raise exception 'NFL Build a QB did not complete as an eight-round four-trait game: %', row_to_json(v_state);
  end if;
  ...
  if exists (
    ...
    raise exception 'completed NFL Build a QB unexpectedly awarded Clutch';
  end if;
  ...
  if not exists (
    ...
    raise exception 'NFL Draft Room completion did not persist through the canonical challenge record';
  end if;
end $$;
