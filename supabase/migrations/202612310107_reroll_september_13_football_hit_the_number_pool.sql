-- Same-day Football Daily refresh for the capped Hit the Number cleanup.
-- Preserve the canonical Football Daily runtime and game rotation while advancing
-- the immutable schedule identity so September 13 rematerializes through the new
-- capped/curated Football Hit the Number generator.
do $$
declare
  v_target_day constant date := date '2026-09-13';
  v_source_version constant text := 'football-daily-v6-question-refresh';
  v_replacement_version constant text := 'football-daily-v7-hit-number-pool-cleanup';
...
begin
  ...
  if v_existing_daily_id is not null then
    if v_central_today <> v_target_day then
      raise exception 'September 13 Football Daily pool refresh is only safe on %, current Central day is %',
        v_target_day,
        v_central_today;
    end if;
  ...
  if not found then
    raise exception 'source Football Daily schedule % disappeared during pool refresh', v_source_version;
  end if;
