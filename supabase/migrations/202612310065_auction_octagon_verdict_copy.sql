-- Build the Ultimate Fighter handoff belongs to Octagon Verdict, not the app.
-- Keep the participant-safe coded packet and remove the private comparison/narration
-- helpers introduced after that packet. Historical migrations remain immutable.
...
comment on function public.get_auction_fight_breakdown_packet(uuid) is
  'Participant-only completed Build the Ultimate Fighter copy packet for Octagon Verdict. Returns public result data plus opaque private-rating codes; never returns hidden ratings, rating-code mapping, matchup analysis, or narration.';
...
-- #704 depends on #702, so remove the narration owner first and then the comparison owner.
do $$
begin
  if to_regprocedure('private.auction_ultimate_fighter_recap(uuid)') is null then
    raise exception 'Auction Octagon Verdict cleanup drifted: recap owner missing';
  end if;
  if to_regprocedure('private.auction_ultimate_fighter_analysis(uuid)') is null then
    raise exception 'Auction Octagon Verdict cleanup drifted: analysis owner missing';
  end if;
end;
$$;
