...
begin
  ...
  loop
    ...
    if (
      select count(*) from jsonb_array_elements_text(v_value) where trim(value) <> ''
    ) <> (
      select count(distinct lower(trim(value))) from jsonb_array_elements_text(v_value) where trim(value) <> ''
    ) then
...
create or replace function public.get_football_futures(p_season integer)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  if p_season is null or p_season < 1993 or p_season > 2200 then raise exception 'football_futures_invalid_season'; end if;
  v_lock_at := public.football_futures_lock_at(p_season);

  select row.picks into v_own
  from public.football_futures_picks row
  where row.profile_id = v_profile_id and row.season = p_season;
...
begin
  if v_profile_id is null then raise exception 'sign in required'; end if;
  if p_season is null or p_season < 1993 or p_season > 2200 then raise exception 'football_futures_invalid_season'; end if;
  if now() >= public.football_futures_lock_at(p_season) then raise exception 'football_futures_locked'; end if;
  ...
  return public.get_football_futures(p_season);
end;
$$;
...
revoke all on function public.get_football_futures(integer) from public, anon;
revoke all on function public.save_football_futures(integer, jsonb) from public, anon;
grant execute on function public.get_football_futures(integer) to authenticated;
