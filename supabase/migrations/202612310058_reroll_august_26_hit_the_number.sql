-- One-time same-day replacement for the August 26, 2026 official Hit the Number challenge.
-- Preserve the canonical Daily Challenge runtime: a new immutable schedule identity changes the
-- hidden deterministic seed and lets the existing runtime materialize a fresh board.
do $$
declare
  v_target_day constant date := date '2026-08-26';
  v_source_version constant text := 'play-rotation-v4';
  v_replacement_version constant text := 'play-rotation-v5';
...
begin
  ...
  if v_expected_game is distinct from 'hit_the_number' then
    raise exception 'expected Hit the Number on %, found %',
      v_target_day,
  ...
  -- Fresh database replays have no materialized historical row and simply inherit v5 onward.
  if v_existing_daily_id is not null then
    if v_central_today <> v_target_day then
      raise exception 'August 26 Daily Challenge replacement is only safe on %, current Central day is %',
        v_target_day,
        v_central_today;
    end if;
