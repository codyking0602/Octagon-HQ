...
type SecretFootballTab = "picks" | "play";
...
const SECRET_FOOTBALL_TAP_WINDOW_MS = 350;
...
export function BottomNavigation({ themeScope = "neutral" }: { themeScope?: HqThemeScope }) {
  ...
  const lastActiveTabTapRef = useRef<Record<SecretFootballTab, number>>({ picks: 0, play: 0 });
        ...
        <NavLink
          ...
          onClick={(event) => {
            if (destination.icon === "play" || destination.icon === "picks") {
              const secretTab = destination.icon;
              const activeTab = routeOwnsNavigationItem(secretTab, location.pathname);
              if (activeTab) {
                const now = Date.now();
                if (now - lastActiveTabTapRef.current[secretTab] <= SECRET_FOOTBALL_TAP_WINDOW_MS) {
                  event.preventDefault();
                  lastActiveTabTapRef.current[secretTab] = 0;
                  if (footballMode) {
                    setSelectedSport("ufc");
                    navigate(secretTab === "play" ? "/play" : "/picks");
                  } else {
                    setSelectedSport("football");
                    navigate(secretTab === "play" ? "/football" : "/football/picks", {
                      state: { footballTabEntry: secretTab === "play" ? "vince" : "zeke" },
                    });
                  }
                  return;
                }
                lastActiveTabTapRef.current[secretTab] = now;
              }
            }
