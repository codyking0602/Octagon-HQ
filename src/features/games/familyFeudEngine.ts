...
function personNameTokens(displayName: string) {
...
function personFirstName(displayName: string) {
  return personNameTokens(displayName)[0] ?? "";
}

function personSurname(displayName: string) {
  return personNameTokens(displayName).at(-1) ?? "";
}
...
function typoAllowance(value: string) {
  if (value.includes(" ")) {
    ...
    if (value.length < 18) return 4;
    return 6;
  }
  if (value.length < 5) return 0;
  if (value.length < 9) return 1;
  if (value.length < 13) return 2;
  return 3;
}
...
export function familyFeudMainAnswerPoints(
  question: FamilyFeudQuestion,
  entityId: string,
) {
  ...
  if ((question.alsoAcceptedEntityIds ?? []).includes(entityId)) {
    return FAMILY_FEUD_MAIN_ALSO_ACCEPTED_POINTS;
  }
  return null;
}
...
export function familyFeudMainBoardScore(
  ...
  return board.revealedEntityIds.reduce(
    (sum, entityId) => sum + (familyFeudMainAnswerPoints(question, entityId) ?? 0),
    0,
  );
}
