...
try {
  ...
  const preview = await request(
    ...
    `${supabaseOrigin}/functions/v1/sync-next-ufc-event`,
    {
      ...
      headers: {
        ...
        "Content-Type": "application/json",
        Origin: productionOrigin,
        "x-client-info": "octagon-hq-event-preview-check/1",
      },
  ...
  if (!Number.isInteger(preview.body?.fight_count) || preview.body.fight_count < 4 || preview.body.fight_count > 8) {
    throw new Error(`Preview returned an implausible Fight Night main-card count: ${preview.body?.fight_count ?? "missing"}.`);
  }
  ...
  const draftAfter = (await rpc("get_pick_event_setup", userToken)).body;
  const liveAfter = (await rpc("get_current_pick_event", userToken)).body;
  if (JSON.stringify(draftAfter) !== JSON.stringify(draftBefore)) {
  ...
  console.log(
    `PASS: production Event Setup preview returned ${preview.body.fight_count} main-card fights from the supplied MMA Mania URL at exact backend ${expectedSha}; staged and live cards were unchanged.`,
  );
} finally {
  if (userId) {
