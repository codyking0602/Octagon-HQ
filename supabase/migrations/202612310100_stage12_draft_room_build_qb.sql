...
insert into private.auction_catalog (
  ...
  ('football-draft-room-2026-09-v1','build-qb','nfl-patrick-mahomes','Patrick Mahomes',4,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',95,'Accuracy',88,'Processing',87,'Mobility',83,'Clutch',94,'overall',89)),
  ('football-draft-room-2026-09-v1','build-qb','nfl-aaron-rodgers','Aaron Rodgers',4,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',74,'Accuracy',89,'Processing',99,'Mobility',75,'Clutch',98,'overall',87)),
  ('football-draft-room-2026-09-v1','build-qb','nfl-lamar-jackson','Lamar Jackson',4,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',67,'Accuracy',82,'Processing',90,'Mobility',96,'Clutch',93,'overall',86)),
  ('football-draft-room-2026-09-v1','build-qb','nfl-joe-burrow','Joe Burrow',4,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',79,'Accuracy',95,'Processing',88,'Mobility',74,'Clutch',77,'overall',83)),
  ('football-draft-room-2026-09-v1','build-qb','drew-brees','Drew Brees',3,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',88,'Accuracy',88,'Processing',73,'Mobility',50,'Clutch',89,'overall',78)),
  ('football-draft-room-2026-09-v1','build-qb','tom-brady','Tom Brady',3,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',69,'Accuracy',69,'Processing',84,'Mobility',52,'Clutch',92,'overall',73)),
  ('football-draft-room-2026-09-v1','build-qb','nfl-josh-allen','Josh Allen',3,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',55,'Accuracy',62,'Processing',69,'Mobility',93,'Clutch',83,'overall',72)),
  ('football-draft-room-2026-09-v1','build-qb','matt-ryan','Matt Ryan',2,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',68,'Accuracy',75,'Processing',67,'Mobility',57,'Clutch',83,'overall',70)),
  ('football-draft-room-2026-09-v1','build-qb','nfl-matthew-stafford','Matthew Stafford',2,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',68,'Accuracy',53,'Processing',65,'Mobility',56,'Clutch',81,'overall',65)),
  ('football-draft-room-2026-09-v1','build-qb','nfl-philip-rivers','Philip Rivers',2,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',78,'Accuracy',74,'Processing',56,'Mobility',38,'Clutch',80,'overall',65)),
  ('football-draft-room-2026-09-v1','build-qb','andrew-luck','Andrew Luck',2,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',67,'Accuracy',46,'Processing',54,'Mobility',82,'Clutch',72,'overall',64)),
  ('football-draft-room-2026-09-v1','build-qb','ben-roethlisberger','Ben Roethlisberger',2,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',70,'Accuracy',63,'Processing',54,'Mobility',55,'Clutch',78,'overall',64)),
  ('football-draft-room-2026-09-v1','build-qb','cam-newton','Cam Newton',1,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',43,'Accuracy',37,'Processing',43,'Mobility',93,'Clutch',68,'overall',57)),
  ('football-draft-room-2026-09-v1','build-qb','jay-cutler','Jay Cutler',1,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',41,'Accuracy',47,'Processing',37,'Mobility',62,'Clutch',65,'overall',50)),
  ('football-draft-room-2026-09-v1','build-qb','eli-manning','Eli Manning',1,'Canonical NFL QB profile from Football position-trait model v1',1.0,'qb',jsonb_build_object('Arm',43,'Accuracy',38,'Processing',38,'Mobility',38,'Clutch',69,'overall',45));

-- Extend the existing sealed-bid storage invariants for the Build a QB format.
-- This keeps one Auction/Draft Room backend owner while preserving every UFC limit.
...
begin
  ...
  if tg_table_name = 'auction_deck_entries' then
    if new.deck_position > (
      case
        when v_auction.mode_id in ('ultimate-fighter', 'build-qb') then 10
  ...
  if tg_table_name = 'auction_pending_bids' then
    ...
    if v_auction.mode_id not in ('ultimate-fighter', 'build-qb')
...
begin
  if new.challenge_id is not null and not exists (
    ...
    where challenge.id = new.challenge_id
      and challenge.game_id = private.auction_game_id_for_mode(new.mode_id)
...
begin
  if exists (
    ...
    where auction.challenge_id = new.id
      and (
        new.game_id <> private.auction_game_id_for_mode(auction.mode_id)
...
begin
  if old.declined_at is null and new.declined_at is not null then
    ...
    if found then
      ...
      v_is_draft := v_auction.mode_id = 'build-qb';
...
-- Generic Challenge Center commands must not become a second Draft Room owner.
do $$
declare
  v_definition text;
  v_expected text;
begin
  v_definition := pg_get_functiondef(
    'public.create_play_challenge(uuid,text,text,text,text,text,jsonb,jsonb)'::regprocedure
  );
  v_expected := 'if trim(p_game_id) = ''auction'' then';
  if position(v_expected in v_definition) = 0 then
    raise exception 'Draft Room owner drifted: create_play_challenge';
  end if;
  v_definition := replace(
    v_definition,
    v_expected,
    'if trim(p_game_id) in (''auction'', ''draft-room'') then'
  );
  execute v_definition;

  v_definition := pg_get_functiondef('public.open_play_challenge(text)'::regprocedure);
  v_expected := 'if v_challenge.game_id = ''auction'' then';
  if position(v_expected in v_definition) = 0 then
    raise exception 'Draft Room owner drifted: open_play_challenge';
  end if;
  v_definition := replace(
    v_definition,
    v_expected,
    'if v_challenge.game_id in (''auction'', ''draft-room'') then'
  );
  execute v_definition;

  v_definition := pg_get_functiondef('public.complete_play_challenge(text,jsonb)'::regprocedure);
  v_expected := 'if v_challenge.game_id = ''auction'' then';
  if position(v_expected in v_definition) = 0 then
    raise exception 'Draft Room owner drifted: complete_play_challenge';
  end if;
  v_definition := replace(
    v_definition,
    v_expected,
