...
const expectedMainSha = (process.env.EXPECTED_LIVE_MAIN_SHA ?? "").trim();
...
if (!secretKey) {
  throw new Error("Project service key is unavailable.");
}
...
try {
  ...
  console.log(
    `PASS: live Control Center displayed five Gamrot vs. Quillan deadlines at 390x844: ${visibleDeadlines.join(" | ")}.`,
  );
} catch (error) {
  primaryError = error;
} finally {
