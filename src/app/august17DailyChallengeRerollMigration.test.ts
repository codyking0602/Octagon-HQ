...
describe("August 17 Daily Challenge reroll migration", () => {
  it("hard-limits the live replacement to the intended Central day and active schedule", () => {
    ...
    expect(migration).toContain("if v_existing_daily_id is null then");
  });

  it("keeps fresh database replays valid without inventing a second runtime owner", () => {
    expect(migration).toContain("v_replacement_starts_on := greatest(v_target_day, v_source_starts_on)");
    expect(migration).toContain("v_replacement_starts_on,");
    expect(migration).toContain("elsif v_existing_daily_id is not null then");
