...
describe("draft fast feedback and final validation", () => {
  it("reserves the full validation lanes for ready PRs and main pushes", () => {
    ...
    expect(fullValidation.match(new RegExp(readyOrMain.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"))).toHaveLength(5);
    expect(fullValidation).toContain("types: [opened, reopened, ready_for_review]");
    expect(fullValidation).not.toContain("types: [opened, synchronize, reopened, ready_for_review]");
    expect(fullValidation).toContain("shard: [1, 2, 3, 4, 5, 6, 7, 8]");
    expect(fullValidation).toContain("name: Football ESPN schedule provider");
    expect(fullValidation).toContain("node scripts/verify-football-espn-schedule-live.mjs");
