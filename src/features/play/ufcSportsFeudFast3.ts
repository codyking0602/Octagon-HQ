import { expandSportsFeudFamilies } from "./sportsFeudAuthoredHelpers";

export const UFC_SPORTS_FEUD_FAST_3 = expandSportsFeudFamilies("ufc-fast3", [
  {
    "category": "champion-legacy",
    "entityKind": "person",
    "collisionGroup": "champions",
    "prompts": [
      {
        "prompt": "Name a UFC champion known for a long or dominant reign.",
        "answers": [
          "Anderson Silva",
          "Georges St-Pierre",
          "Demetrious Johnson",
          "Jon Jones",
          "Jose Aldo",
          "Amanda Nunes",
          "Valentina Shevchenko",
          "Israel Adesanya"
        ],
        "alsoAcceptedAnswers": [
          "Khabib Nurmagomedov",
          "Alexander Volkanovski",
          "Kamaru Usman",
          "Matt Hughes",
          "Ronda Rousey",
          "Islam Makhachev"
        ]
      },
      {
        "prompt": "Name a fighter who defended a UFC title repeatedly.",
        "answers": [
          "Demetrious Johnson",
          "Anderson Silva",
          "Georges St-Pierre",
          "Jon Jones",
          "Jose Aldo",
          "Amanda Nunes",
          "Valentina Shevchenko",
          "Israel Adesanya"
        ],
        "alsoAcceptedAnswers": [
          "Khabib Nurmagomedov",
          "Alexander Volkanovski",
          "Kamaru Usman",
          "Matt Hughes",
          "Ronda Rousey",
          "Islam Makhachev"
        ]
      },
      {
        "prompt": "Name a champion who made a division feel like it belonged to them.",
        "answers": [
          "Anderson Silva",
          "Georges St-Pierre",
          "Jon Jones",
          "Demetrious Johnson",
          "Jose Aldo",
          "Amanda Nunes",
          "Khabib Nurmagomedov",
          "Kamaru Usman"
        ],
        "alsoAcceptedAnswers": [
          "Valentina Shevchenko",
          "Israel Adesanya",
          "Alexander Volkanovski",
          "Matt Hughes",
          "Ronda Rousey",
          "Islam Makhachev"
        ]
      },
      {
        "prompt": "Name a UFC titleholder associated with sustained dominance.",
        "answers": [
          "Anderson Silva",
          "Georges St-Pierre",
          "Jon Jones",
          "Demetrious Johnson",
          "Jose Aldo",
          "Amanda Nunes",
          "Valentina Shevchenko",
          "Alexander Volkanovski"
        ],
        "alsoAcceptedAnswers": [
          "Israel Adesanya",
          "Khabib Nurmagomedov",
          "Kamaru Usman",
          "Matt Hughes",
          "Ronda Rousey",
          "Islam Makhachev"
        ]
      },
      {
        "prompt": "Name a champion whose reign became a major part of UFC history.",
        "answers": [
          "Anderson Silva",
          "Georges St-Pierre",
          "Jon Jones",
          "Demetrious Johnson",
          "Jose Aldo",
          "Amanda Nunes",
          "Israel Adesanya",
          "Khabib Nurmagomedov"
        ],
        "alsoAcceptedAnswers": [
          "Valentina Shevchenko",
          "Alexander Volkanovski",
          "Kamaru Usman",
          "Matt Hughes",
          "Ronda Rousey",
          "Islam Makhachev"
        ]
      }
    ],
    "answers": [
      {
        "name": "Anderson Silva",
        "aliases": [
          "The Spider"
        ]
      },
      {
        "name": "Georges St-Pierre",
        "aliases": [
          "GSP",
          "St Pierre",
          "Georges St Pierre"
        ]
      },
      {
        "name": "Demetrious Johnson",
        "aliases": [
          "Mighty Mouse",
          "DJ"
        ]
      },
      {
        "name": "Jon Jones",
        "aliases": [
          "Bones",
          "Jones"
        ]
      },
      {
        "name": "Jose Aldo",
        "aliases": [
          "Aldo"
        ]
      },
      {
        "name": "Amanda Nunes",
        "aliases": [
          "The Lioness"
        ]
      },
      {
        "name": "Valentina Shevchenko",
        "aliases": [
          "Bullet"
        ]
      },
      {
        "name": "Israel Adesanya",
        "aliases": [
          "Izzy",
          "Stylebender"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Khabib Nurmagomedov",
        "aliases": [
          "Khabib"
        ]
      },
      {
        "name": "Alexander Volkanovski",
        "aliases": [
          "Volk"
        ]
      },
      {
        "name": "Kamaru Usman",
        "aliases": [
          "Usman"
        ]
      },
      {
        "name": "Matt Hughes",
        "aliases": [
          "Hughes"
        ]
      },
      {
        "name": "Ronda Rousey",
        "aliases": [
          "Rowdy"
        ]
      },
      "Islam Makhachev"
    ]
  },
  {
    "category": "ufc-history",
    "entityKind": "person",
    "collisionGroup": "tuf",
    "prompts": [
      {
        "prompt": "Name a fighter who became famous through The Ultimate Fighter.",
        "answers": [
          "Forrest Griffin",
          "Stephan Bonnar",
          "Michael Bisping",
          "Rashad Evans",
          "Tony Ferguson",
          "Nate Diaz",
          "Matt Serra",
          "Robert Whittaker"
        ],
        "alsoAcceptedAnswers": [
          "Diego Sanchez",
          "Kenny Florian",
          "Gray Maynard",
          "Kelvin Gastelum",
          "Rose Namajunas",
          "Carla Esparza",
          "Julianna Pena",
          "Uriah Hall",
          "Chris Leben",
          "Kamaru Usman"
        ]
      },
      {
        "prompt": "Name a notable UFC fighter who appeared on The Ultimate Fighter.",
        "answers": [
          "Forrest Griffin",
          "Michael Bisping",
          "Rashad Evans",
          "Tony Ferguson",
          "Nate Diaz",
          "Matt Serra",
          "Robert Whittaker",
          "Rose Namajunas"
        ],
        "alsoAcceptedAnswers": [
          "Stephan Bonnar",
          "Diego Sanchez",
          "Kenny Florian",
          "Gray Maynard",
          "Kelvin Gastelum",
          "Carla Esparza",
          "Julianna Pena",
          "Uriah Hall",
          "Chris Leben",
          "Kamaru Usman"
        ]
      },
      {
        "prompt": "Name a UFC name strongly connected with TUF.",
        "answers": [
          "Forrest Griffin",
          "Stephan Bonnar",
          "Michael Bisping",
          "Rashad Evans",
          "Tony Ferguson",
          "Nate Diaz",
          "Diego Sanchez",
          "Matt Serra"
        ],
        "alsoAcceptedAnswers": [
          "Robert Whittaker",
          "Kenny Florian",
          "Gray Maynard",
          "Kelvin Gastelum",
          "Rose Namajunas",
          "Carla Esparza",
          "Julianna Pena",
          "Uriah Hall",
          "Chris Leben",
          "Kamaru Usman"
        ]
      },
      {
        "prompt": "Name a fighter whose career got a major boost from The Ultimate Fighter.",
        "answers": [
          "Forrest Griffin",
          "Michael Bisping",
          "Rashad Evans",
          "Tony Ferguson",
          "Nate Diaz",
          "Matt Serra",
          "Robert Whittaker",
          "Kelvin Gastelum"
        ],
        "alsoAcceptedAnswers": [
          "Stephan Bonnar",
          "Diego Sanchez",
          "Kenny Florian",
          "Gray Maynard",
          "Rose Namajunas",
          "Carla Esparza",
          "Julianna Pena",
          "Uriah Hall",
          "Chris Leben",
          "Kamaru Usman"
        ]
      },
      {
        "prompt": "Name a TUF alum MMA fans still remember.",
        "answers": [
          "Forrest Griffin",
          "Stephan Bonnar",
          "Michael Bisping",
          "Rashad Evans",
          "Tony Ferguson",
          "Nate Diaz",
          "Diego Sanchez",
          "Kenny Florian"
        ],
        "alsoAcceptedAnswers": [
          "Matt Serra",
          "Robert Whittaker",
          "Gray Maynard",
          "Kelvin Gastelum",
          "Rose Namajunas",
          "Carla Esparza",
          "Julianna Pena",
          "Uriah Hall",
          "Chris Leben",
          "Kamaru Usman"
        ]
      }
    ],
    "answers": [
      {
        "name": "Forrest Griffin",
        "aliases": [
          "Forrest"
        ]
      },
      {
        "name": "Stephan Bonnar",
        "aliases": [
          "Bonnar"
        ]
      },
      {
        "name": "Michael Bisping",
        "aliases": [
          "The Count"
        ]
      },
      {
        "name": "Rashad Evans",
        "aliases": [
          "Suga"
        ]
      },
      {
        "name": "Tony Ferguson",
        "aliases": [
          "El Cucuy"
        ]
      },
      {
        "name": "Nate Diaz",
        "aliases": [
          "Nate"
        ]
      },
      {
        "name": "Matt Serra",
        "aliases": [
          "Serra"
        ]
      },
      {
        "name": "Robert Whittaker",
        "aliases": [
          "Bobby Knuckles",
          "The Reaper"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Diego Sanchez",
        "aliases": [
          "Nightmare"
        ]
      },
      {
        "name": "Kenny Florian",
        "aliases": [
          "KenFlo"
        ]
      },
      {
        "name": "Gray Maynard",
        "aliases": [
          "Bully"
        ]
      },
      {
        "name": "Kelvin Gastelum",
        "aliases": [
          "Gastelum"
        ]
      },
      {
        "name": "Rose Namajunas",
        "aliases": [
          "Thug Rose"
        ]
      },
      {
        "name": "Carla Esparza",
        "aliases": [
          "Cookie Monster"
        ]
      },
      {
        "name": "Julianna Pena",
        "aliases": [
          "Pena",
          "Peña"
        ]
      },
      {
        "name": "Uriah Hall",
        "aliases": [
          "Prime Time"
        ]
      },
      {
        "name": "Chris Leben",
        "aliases": [
          "Crippler"
        ]
      },
      {
        "name": "Kamaru Usman",
        "aliases": [
          "Usman"
        ]
      }
    ]
  },
  {
    "category": "rivalries",
    "entityKind": "other",
    "collisionGroup": "rivalries",
    "prompts": [
      {
        "prompt": "Name a famous UFC rivalry.",
        "answers": [
          "Jones-Cormier",
          "McGregor-Nurmagomedov",
          "McGregor-Diaz",
          "Liddell-Ortiz",
          "Silva-Sonnen",
          "Adesanya-Pereira",
          "Ortiz-Shamrock",
          "Usman-Covington"
        ],
        "alsoAcceptedAnswers": [
          "Hughes-St-Pierre",
          "Couture-Liddell",
          "McGregor-Aldo",
          "Nunes-Shevchenko",
          "Poirier-McGregor"
        ]
      },
      {
        "prompt": "Name a pair of fighters whose bad blood made the fight bigger.",
        "answers": [
          "Jones-Cormier",
          "McGregor-Nurmagomedov",
          "Liddell-Ortiz",
          "Ortiz-Shamrock",
          "Usman-Covington",
          "McGregor-Diaz",
          "Poirier-McGregor",
          "Silva-Sonnen"
        ],
        "alsoAcceptedAnswers": [
          "Hughes-St-Pierre",
          "Couture-Liddell",
          "Adesanya-Pereira",
          "McGregor-Aldo",
          "Nunes-Shevchenko"
        ]
      },
      {
        "prompt": "Name a UFC feud fans still remember.",
        "answers": [
          "Jones-Cormier",
          "McGregor-Diaz",
          "Liddell-Ortiz",
          "Silva-Sonnen",
          "McGregor-Nurmagomedov",
          "Ortiz-Shamrock",
          "Adesanya-Pereira",
          "Hughes-St-Pierre"
        ],
        "alsoAcceptedAnswers": [
          "Couture-Liddell",
          "McGregor-Aldo",
          "Nunes-Shevchenko",
          "Usman-Covington",
          "Poirier-McGregor"
        ]
      },
      {
        "prompt": "Name a rivalry that produced major UFC promotion and drama.",
        "answers": [
          "McGregor-Nurmagomedov",
          "Jones-Cormier",
          "McGregor-Diaz",
          "Liddell-Ortiz",
          "Usman-Covington",
          "Poirier-McGregor",
          "Adesanya-Pereira",
          "Ortiz-Shamrock"
        ],
        "alsoAcceptedAnswers": [
          "Silva-Sonnen",
          "Hughes-St-Pierre",
          "Couture-Liddell",
          "McGregor-Aldo",
          "Nunes-Shevchenko"
        ]
      },
      {
        "prompt": "Name two fighters whose rivalry became part of UFC history.",
        "answers": [
          "Jones-Cormier",
          "Liddell-Ortiz",
          "McGregor-Diaz",
          "Silva-Sonnen",
          "McGregor-Nurmagomedov",
          "Hughes-St-Pierre",
          "Couture-Liddell",
          "Adesanya-Pereira"
        ],
        "alsoAcceptedAnswers": [
          "Ortiz-Shamrock",
          "McGregor-Aldo",
          "Nunes-Shevchenko",
          "Usman-Covington",
          "Poirier-McGregor"
        ]
      }
    ],
    "answers": [
      {
        "name": "McGregor-Diaz",
        "aliases": [
          "McGregor vs Diaz",
          "Conor vs Nate"
        ]
      },
      {
        "name": "Jones-Cormier",
        "aliases": [
          "Jones vs Cormier",
          "Jones DC"
        ]
      },
      {
        "name": "Liddell-Ortiz",
        "aliases": [
          "Liddell vs Ortiz",
          "Chuck vs Tito"
        ]
      },
      {
        "name": "Silva-Sonnen",
        "aliases": [
          "Silva vs Sonnen"
        ]
      },
      {
        "name": "McGregor-Nurmagomedov",
        "aliases": [
          "McGregor vs Khabib",
          "Conor vs Khabib"
        ]
      },
      {
        "name": "Hughes-St-Pierre",
        "aliases": [
          "Hughes vs GSP"
        ]
      },
      {
        "name": "Couture-Liddell",
        "aliases": [
          "Couture vs Liddell"
        ]
      },
      {
        "name": "Adesanya-Pereira",
        "aliases": [
          "Adesanya vs Pereira",
          "Izzy vs Pereira"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Ortiz-Shamrock",
        "aliases": [
          "Ortiz vs Shamrock"
        ]
      },
      {
        "name": "McGregor-Aldo",
        "aliases": [
          "McGregor vs Aldo"
        ]
      },
      {
        "name": "Nunes-Shevchenko",
        "aliases": [
          "Nunes vs Shevchenko"
        ]
      },
      {
        "name": "Usman-Covington",
        "aliases": [
          "Usman vs Covington"
        ]
      },
      {
        "name": "Poirier-McGregor",
        "aliases": [
          "Poirier vs McGregor"
        ]
      }
    ]
  },
  {
    "category": "fights",
    "entityKind": "other",
    "collisionGroup": "fights",
    "prompts": [
      {
        "prompt": "Name an iconic UFC fight.",
        "answers": [
          "Griffin-Bonnar",
          "Jones-Gustafsson 1",
          "Lawler-MacDonald 2",
          "Henderson-Shogun 1",
          "Zhang-Jedrzejczyk 1",
          "Adesanya-Gastelum",
          "McGregor-Diaz 2",
          "Silva-Sonnen 1"
        ],
        "alsoAcceptedAnswers": [
          "Poirier-Hooker",
          "Edgar-Maynard 2",
          "Hunt-Silva",
          "Usman-Covington 1",
          "Gaethje-Chandler",
          "Holloway-Kattar",
          "Holloway-Gaethje",
          "Whittaker-Romero 2",
          "Gaethje-Topuria"
        ]
      },
      {
        "prompt": "Name a UFC fight you would show a new MMA fan.",
        "answers": [
          "Griffin-Bonnar",
          "Lawler-MacDonald 2",
          "Zhang-Jedrzejczyk 1",
          "Gaethje-Chandler",
          "Holloway-Kattar",
          "Poirier-Hooker",
          "Hunt-Silva",
          "Adesanya-Gastelum"
        ],
        "alsoAcceptedAnswers": [
          "Jones-Gustafsson 1",
          "Henderson-Shogun 1",
          "McGregor-Diaz 2",
          "Silva-Sonnen 1",
          "Edgar-Maynard 2",
          "Usman-Covington 1",
          "Holloway-Gaethje",
          "Whittaker-Romero 2",
          "Gaethje-Topuria"
        ]
      },
      {
        "prompt": "Name a bout that belongs in a UFC history montage.",
        "answers": [
          "Griffin-Bonnar",
          "Jones-Gustafsson 1",
          "Lawler-MacDonald 2",
          "Henderson-Shogun 1",
          "Silva-Sonnen 1",
          "McGregor-Diaz 2",
          "Zhang-Jedrzejczyk 1",
          "Holloway-Gaethje"
        ],
        "alsoAcceptedAnswers": [
          "Adesanya-Gastelum",
          "Poirier-Hooker",
          "Edgar-Maynard 2",
          "Hunt-Silva",
          "Usman-Covington 1",
          "Gaethje-Chandler",
          "Holloway-Kattar",
          "Whittaker-Romero 2",
          "Gaethje-Topuria"
        ]
      },
      {
        "prompt": "Name a fight remembered for incredible action or stakes.",
        "answers": [
          "Lawler-MacDonald 2",
          "Jones-Gustafsson 1",
          "Henderson-Shogun 1",
          "Zhang-Jedrzejczyk 1",
          "Adesanya-Gastelum",
          "McGregor-Diaz 2",
          "Usman-Covington 1",
          "Whittaker-Romero 2"
        ],
        "alsoAcceptedAnswers": [
          "Griffin-Bonnar",
          "Poirier-Hooker",
          "Silva-Sonnen 1",
          "Edgar-Maynard 2",
          "Hunt-Silva",
          "Gaethje-Chandler",
          "Holloway-Kattar",
          "Holloway-Gaethje",
          "Gaethje-Topuria"
        ]
      },
      {
        "prompt": "Name a UFC matchup people still talk about years later.",
        "answers": [
          "Griffin-Bonnar",
          "Jones-Gustafsson 1",
          "Lawler-MacDonald 2",
          "McGregor-Diaz 2",
          "Silva-Sonnen 1",
          "Henderson-Shogun 1",
          "Zhang-Jedrzejczyk 1",
          "Adesanya-Gastelum"
        ],
        "alsoAcceptedAnswers": [
          "Poirier-Hooker",
          "Edgar-Maynard 2",
          "Hunt-Silva",
          "Usman-Covington 1",
          "Gaethje-Chandler",
          "Holloway-Kattar",
          "Holloway-Gaethje",
          "Whittaker-Romero 2",
          "Gaethje-Topuria"
        ]
      }
    ],
    "answers": [
      {
        "name": "Griffin-Bonnar",
        "aliases": [
          "Griffin vs Bonnar"
        ]
      },
      {
        "name": "Jones-Gustafsson 1",
        "aliases": [
          "Jones vs Gustafsson 1",
          "Jones Gustafsson"
        ]
      },
      {
        "name": "Lawler-MacDonald 2",
        "aliases": [
          "Lawler vs MacDonald 2",
          "Lawler Rory 2"
        ]
      },
      {
        "name": "Henderson-Shogun 1",
        "aliases": [
          "Henderson vs Shogun 1",
          "Hendo Shogun"
        ]
      },
      {
        "name": "Adesanya-Gastelum",
        "aliases": [
          "Adesanya vs Gastelum",
          "Izzy Gastelum"
        ]
      },
      {
        "name": "Zhang-Jedrzejczyk 1",
        "aliases": [
          "Zhang vs Joanna 1",
          "Weili Joanna"
        ]
      },
      {
        "name": "McGregor-Diaz 2",
        "aliases": [
          "McGregor vs Diaz 2",
          "Conor Nate 2"
        ]
      },
      {
        "name": "Poirier-Hooker",
        "aliases": [
          "Poirier vs Hooker"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Silva-Sonnen 1",
        "aliases": [
          "Silva vs Sonnen 1"
        ]
      },
      {
        "name": "Edgar-Maynard 2",
        "aliases": [
          "Edgar vs Maynard 2"
        ]
      },
      {
        "name": "Hunt-Silva",
        "aliases": [
          "Hunt vs Silva"
        ]
      },
      {
        "name": "Usman-Covington 1",
        "aliases": [
          "Usman vs Covington 1"
        ]
      },
      {
        "name": "Gaethje-Chandler",
        "aliases": [
          "Gaethje vs Chandler"
        ]
      },
      {
        "name": "Holloway-Kattar",
        "aliases": [
          "Holloway vs Kattar"
        ]
      },
      "Holloway-Gaethje",
      {
        "name": "Whittaker-Romero 2",
        "aliases": [
          "Whittaker vs Romero 2",
          "Whittaker Romero 2"
        ]
      },
      {
        "name": "Gaethje-Topuria",
        "aliases": [
          "Gaethje vs Topuria",
          "Justin vs Ilia",
          "Gaethje Topuria"
        ]
      }
    ]
  },
  {
    "category": "ufc-history",
    "entityKind": "person",
    "collisionGroup": "pioneers",
    "prompts": [
      {
        "prompt": "Name an early UFC pioneer.",
        "answers": [
          "Royce Gracie",
          "Ken Shamrock",
          "Dan Severn",
          "Mark Coleman",
          "Tank Abbott",
          "Don Frye",
          "Randy Couture",
          "Pat Miletich"
        ],
        "alsoAcceptedAnswers": [
          "Tito Ortiz",
          "Frank Shamrock",
          "Bas Rutten",
          "Vitor Belfort",
          "Oleg Taktarov",
          "Marco Ruas"
        ]
      },
      {
        "prompt": "Name a fighter strongly associated with the UFC's early years.",
        "answers": [
          "Royce Gracie",
          "Ken Shamrock",
          "Dan Severn",
          "Mark Coleman",
          "Tank Abbott",
          "Don Frye",
          "Randy Couture",
          "Tito Ortiz"
        ],
        "alsoAcceptedAnswers": [
          "Pat Miletich",
          "Frank Shamrock",
          "Bas Rutten",
          "Vitor Belfort",
          "Oleg Taktarov",
          "Marco Ruas"
        ]
      },
      {
        "prompt": "Name an old-school UFC name longtime fans recognize.",
        "answers": [
          "Royce Gracie",
          "Ken Shamrock",
          "Dan Severn",
          "Tank Abbott",
          "Don Frye",
          "Mark Coleman",
          "Randy Couture",
          "Oleg Taktarov"
        ],
        "alsoAcceptedAnswers": [
          "Pat Miletich",
          "Tito Ortiz",
          "Frank Shamrock",
          "Bas Rutten",
          "Vitor Belfort",
          "Marco Ruas"
        ]
      },
      {
        "prompt": "Name a fighter who helped establish the UFC before the modern era.",
        "answers": [
          "Royce Gracie",
          "Ken Shamrock",
          "Dan Severn",
          "Mark Coleman",
          "Don Frye",
          "Tank Abbott",
          "Pat Miletich",
          "Vitor Belfort"
        ],
        "alsoAcceptedAnswers": [
          "Randy Couture",
          "Tito Ortiz",
          "Frank Shamrock",
          "Bas Rutten",
          "Oleg Taktarov",
          "Marco Ruas"
        ]
      },
      {
        "prompt": "Name a pioneer you would expect in a conversation about early UFC history.",
        "answers": [
          "Royce Gracie",
          "Ken Shamrock",
          "Dan Severn",
          "Mark Coleman",
          "Don Frye",
          "Randy Couture",
          "Tito Ortiz",
          "Bas Rutten"
        ],
        "alsoAcceptedAnswers": [
          "Tank Abbott",
          "Pat Miletich",
          "Frank Shamrock",
          "Vitor Belfort",
          "Oleg Taktarov",
          "Marco Ruas"
        ]
      }
    ],
    "answers": [
      {
        "name": "Royce Gracie",
        "aliases": [
          "Royce"
        ]
      },
      "Ken Shamrock",
      {
        "name": "Dan Severn",
        "aliases": [
          "The Beast"
        ]
      },
      {
        "name": "Mark Coleman",
        "aliases": [
          "Hammer"
        ]
      },
      {
        "name": "Tank Abbott",
        "aliases": [
          "Tank"
        ]
      },
      {
        "name": "Don Frye",
        "aliases": [
          "Frye"
        ]
      },
      {
        "name": "Randy Couture",
        "aliases": [
          "The Natural"
        ]
      },
      {
        "name": "Pat Miletich",
        "aliases": [
          "Croatian Sensation"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Tito Ortiz",
        "aliases": [
          "Huntington Beach Bad Boy"
        ]
      },
      "Frank Shamrock",
      {
        "name": "Bas Rutten",
        "aliases": [
          "El Guapo"
        ]
      },
      {
        "name": "Vitor Belfort",
        "aliases": [
          "The Phenom"
        ]
      },
      {
        "name": "Oleg Taktarov",
        "aliases": [
          "Russian Bear"
        ]
      },
      {
        "name": "Marco Ruas",
        "aliases": [
          "Ruas"
        ]
      }
    ]
  },
  {
    "category": "mma-history",
    "entityKind": "person",
    "collisionGroup": "pride",
    "prompts": [
      {
        "prompt": "Name a PRIDE star who later fought in the UFC.",
        "answers": [
          "Wanderlei Silva",
          "Mauricio Rua",
          "Dan Henderson",
          "Mirko Cro Cop",
          "Quinton Jackson",
          "Antonio Rodrigo Nogueira",
          "Anderson Silva",
          "Fabricio Werdum"
        ],
        "alsoAcceptedAnswers": [
          "Mark Hunt",
          "Alistair Overeem",
          "Nick Diaz",
          "Ryo Chonan"
        ]
      },
      {
        "prompt": "Name a fighter associated with both PRIDE and the UFC.",
        "answers": [
          "Wanderlei Silva",
          "Mauricio Rua",
          "Dan Henderson",
          "Mirko Cro Cop",
          "Quinton Jackson",
          "Antonio Rodrigo Nogueira",
          "Mark Hunt",
          "Alistair Overeem"
        ],
        "alsoAcceptedAnswers": [
          "Anderson Silva",
          "Fabricio Werdum",
          "Nick Diaz",
          "Kazushi Sakuraba",
          "Ryo Chonan"
        ]
      },
      {
        "prompt": "Name a Japanese-era MMA legend who eventually appeared in the UFC.",
        "answers": [
          "Wanderlei Silva",
          "Mauricio Rua",
          "Mirko Cro Cop",
          "Dan Henderson",
          "Quinton Jackson",
          "Antonio Rodrigo Nogueira",
          "Anderson Silva",
          "Fabricio Werdum"
        ],
        "alsoAcceptedAnswers": [
          "Mark Hunt",
          "Alistair Overeem",
          "Nick Diaz",
          "Ryo Chonan"
        ]
      },
      {
        "prompt": "Name a fighter whose résumé connects PRIDE history with UFC history.",
        "answers": [
          "Wanderlei Silva",
          "Mauricio Rua",
          "Dan Henderson",
          "Mirko Cro Cop",
          "Quinton Jackson",
          "Antonio Rodrigo Nogueira",
          "Anderson Silva",
          "Fabricio Werdum"
        ],
        "alsoAcceptedAnswers": [
          "Mark Hunt",
          "Alistair Overeem",
          "Nick Diaz",
          "Kazushi Sakuraba",
          "Ryo Chonan"
        ]
      },
      {
        "prompt": "Name a former PRIDE standout UFC fans recognize.",
        "answers": [
          "Wanderlei Silva",
          "Mauricio Rua",
          "Dan Henderson",
          "Mirko Cro Cop",
          "Quinton Jackson",
          "Antonio Rodrigo Nogueira",
          "Mark Hunt",
          "Alistair Overeem"
        ],
        "alsoAcceptedAnswers": [
          "Anderson Silva",
          "Fabricio Werdum",
          "Nick Diaz",
          "Kazushi Sakuraba",
          "Ryo Chonan"
        ]
      }
    ],
    "answers": [
      {
        "name": "Wanderlei Silva",
        "aliases": [
          "Axe Murderer"
        ]
      },
      {
        "name": "Mauricio Rua",
        "aliases": [
          "Shogun",
          "Shogun Rua"
        ]
      },
      {
        "name": "Dan Henderson",
        "aliases": [
          "Hendo"
        ]
      },
      {
        "name": "Mirko Cro Cop",
        "aliases": [
          "Cro Cop"
        ]
      },
      {
        "name": "Quinton Jackson",
        "aliases": [
          "Rampage"
        ]
      },
      {
        "name": "Antonio Rodrigo Nogueira",
        "aliases": [
          "Minotauro",
          "Big Nog",
          "Nogueira"
        ]
      },
      {
        "name": "Anderson Silva",
        "aliases": [
          "The Spider"
        ]
      },
      {
        "name": "Fabricio Werdum",
        "aliases": [
          "Vai Cavalo"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Mark Hunt",
        "aliases": [
          "Super Samoan"
        ]
      },
      {
        "name": "Alistair Overeem",
        "aliases": [
          "The Reem"
        ]
      },
      {
        "name": "Nick Diaz",
        "aliases": [
          "Nick"
        ]
      },
      {
        "name": "Kazushi Sakuraba",
        "aliases": [
          "Sakuraba",
          "Gracie Hunter"
        ]
      },
      {
        "name": "Ryo Chonan",
        "aliases": [
          "Chonan"
        ]
      }
    ]
  },
  {
    "category": "ufc-history",
    "entityKind": "person",
    "collisionGroup": "hall-of-fame",
    "prompts": [
      {
        "prompt": "Name a fighter inducted into the UFC Hall of Fame for their career.",
        "answers": [
          "Royce Gracie",
          "Chuck Liddell",
          "Randy Couture",
          "Georges St-Pierre",
          "Khabib Nurmagomedov",
          "BJ Penn",
          "Michael Bisping",
          "Ronda Rousey"
        ],
        "alsoAcceptedAnswers": [
          "Matt Hughes",
          "Tito Ortiz",
          "Urijah Faber",
          "Rashad Evans",
          "Jose Aldo",
          "Anderson Silva",
          "Frankie Edgar",
          "Dominick Cruz",
          "Chris Weidman",
          "Demetrious Johnson",
          "Robbie Lawler",
          "Amanda Nunes",
          "Mauricio Rua",
          "Joanna Jedrzejczyk",
          "Donald Cerrone",
          "Daniel Cormier",
          "Forrest Griffin",
          "Mark Kerr",
          "Vitor Belfort",
          "Wanderlei Silva",
          "Jens Pulver",
          "Kevin Randleman",
          "Rich Franklin",
          "Matt Serra",
          "Kazushi Sakuraba",
          "Maurice Smith",
          "Don Frye",
          "Antonio Rodrigo Nogueira",
          "Bas Rutten",
          "Pat Miletich",
          "Mark Coleman",
          "Dan Severn",
          "Ken Shamrock"
        ]
      },
      {
        "prompt": "Name a UFC Hall of Famer honored for their career.",
        "answers": [
          "Georges St-Pierre",
          "Khabib Nurmagomedov",
          "Anderson Silva",
          "Amanda Nunes",
          "Demetrious Johnson",
          "Jose Aldo",
          "Daniel Cormier",
          "Ronda Rousey"
        ],
        "alsoAcceptedAnswers": [
          "Royce Gracie",
          "Chuck Liddell",
          "Randy Couture",
          "BJ Penn",
          "Michael Bisping",
          "Matt Hughes",
          "Tito Ortiz",
          "Urijah Faber",
          "Rashad Evans",
          "Frankie Edgar",
          "Dominick Cruz",
          "Chris Weidman",
          "Robbie Lawler",
          "Mauricio Rua",
          "Joanna Jedrzejczyk",
          "Donald Cerrone",
          "Forrest Griffin",
          "Mark Kerr",
          "Vitor Belfort",
          "Wanderlei Silva",
          "Jens Pulver",
          "Kevin Randleman",
          "Rich Franklin",
          "Matt Serra",
          "Kazushi Sakuraba",
          "Maurice Smith",
          "Don Frye",
          "Antonio Rodrigo Nogueira",
          "Bas Rutten",
          "Pat Miletich",
          "Mark Coleman",
          "Dan Severn",
          "Ken Shamrock"
        ]
      },
      {
        "prompt": "Name a fighter in the UFC Hall of Fame's Modern or Pioneer Wing.",
        "answers": [
          "Georges St-Pierre",
          "Khabib Nurmagomedov",
          "Demetrious Johnson",
          "Amanda Nunes",
          "Jose Aldo",
          "Daniel Cormier",
          "Michael Bisping",
          "Ronda Rousey"
        ],
        "alsoAcceptedAnswers": [
          "Royce Gracie",
          "Chuck Liddell",
          "Randy Couture",
          "BJ Penn",
          "Matt Hughes",
          "Tito Ortiz",
          "Urijah Faber",
          "Rashad Evans",
          "Anderson Silva",
          "Frankie Edgar",
          "Dominick Cruz",
          "Chris Weidman",
          "Robbie Lawler",
          "Mauricio Rua",
          "Joanna Jedrzejczyk",
          "Donald Cerrone",
          "Forrest Griffin",
          "Mark Kerr",
          "Vitor Belfort",
          "Wanderlei Silva",
          "Jens Pulver",
          "Kevin Randleman",
          "Rich Franklin",
          "Matt Serra",
          "Kazushi Sakuraba",
          "Maurice Smith",
          "Don Frye",
          "Antonio Rodrigo Nogueira",
          "Bas Rutten",
          "Pat Miletich",
          "Mark Coleman",
          "Dan Severn",
          "Ken Shamrock"
        ]
      },
      {
        "prompt": "Name a UFC legend individually inducted into the Hall of Fame.",
        "answers": [
          "Royce Gracie",
          "Chuck Liddell",
          "Randy Couture",
          "Georges St-Pierre",
          "Anderson Silva",
          "Khabib Nurmagomedov",
          "Amanda Nunes",
          "Ronda Rousey"
        ],
        "alsoAcceptedAnswers": [
          "BJ Penn",
          "Michael Bisping",
          "Matt Hughes",
          "Tito Ortiz",
          "Urijah Faber",
          "Rashad Evans",
          "Jose Aldo",
          "Frankie Edgar",
          "Dominick Cruz",
          "Chris Weidman",
          "Demetrious Johnson",
          "Robbie Lawler",
          "Mauricio Rua",
          "Joanna Jedrzejczyk",
          "Donald Cerrone",
          "Daniel Cormier",
          "Forrest Griffin",
          "Mark Kerr",
          "Vitor Belfort",
          "Wanderlei Silva",
          "Jens Pulver",
          "Kevin Randleman",
          "Rich Franklin",
          "Matt Serra",
          "Kazushi Sakuraba",
          "Maurice Smith",
          "Don Frye",
          "Antonio Rodrigo Nogueira",
          "Bas Rutten",
          "Pat Miletich",
          "Mark Coleman",
          "Dan Severn",
          "Ken Shamrock"
        ]
      },
      {
        "prompt": "Name a fighter whose career earned an individual UFC Hall of Fame induction.",
        "answers": [
          "Georges St-Pierre",
          "Anderson Silva",
          "Khabib Nurmagomedov",
          "Amanda Nunes",
          "Demetrious Johnson",
          "Jose Aldo",
          "Chuck Liddell",
          "Randy Couture"
        ],
        "alsoAcceptedAnswers": [
          "Royce Gracie",
          "BJ Penn",
          "Michael Bisping",
          "Ronda Rousey",
          "Matt Hughes",
          "Tito Ortiz",
          "Urijah Faber",
          "Rashad Evans",
          "Frankie Edgar",
          "Dominick Cruz",
          "Chris Weidman",
          "Robbie Lawler",
          "Mauricio Rua",
          "Joanna Jedrzejczyk",
          "Donald Cerrone",
          "Daniel Cormier",
          "Forrest Griffin",
          "Mark Kerr",
          "Vitor Belfort",
          "Wanderlei Silva",
          "Jens Pulver",
          "Kevin Randleman",
          "Rich Franklin",
          "Matt Serra",
          "Kazushi Sakuraba",
          "Maurice Smith",
          "Don Frye",
          "Antonio Rodrigo Nogueira",
          "Bas Rutten",
          "Pat Miletich",
          "Mark Coleman",
          "Dan Severn",
          "Ken Shamrock"
        ]
      }
    ],
    "answers": [
      {
        "name": "Royce Gracie",
        "aliases": [
          "Royce"
        ]
      },
      "Chuck Liddell",
      {
        "name": "Randy Couture",
        "aliases": [
          "The Natural"
        ]
      },
      {
        "name": "Georges St-Pierre",
        "aliases": [
          "GSP",
          "St Pierre",
          "Georges St Pierre"
        ]
      },
      {
        "name": "Khabib Nurmagomedov",
        "aliases": [
          "Khabib"
        ]
      },
      {
        "name": "BJ Penn",
        "aliases": [
          "BJ"
        ]
      },
      {
        "name": "Michael Bisping",
        "aliases": [
          "The Count"
        ]
      },
      {
        "name": "Ronda Rousey",
        "aliases": [
          "Rowdy"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Matt Hughes",
        "aliases": [
          "Hughes"
        ]
      },
      {
        "name": "Tito Ortiz",
        "aliases": [
          "Huntington Beach Bad Boy"
        ]
      },
      {
        "name": "Urijah Faber",
        "aliases": [
          "California Kid"
        ]
      },
      {
        "name": "Rashad Evans",
        "aliases": [
          "Suga"
        ]
      },
      {
        "name": "Jose Aldo",
        "aliases": [
          "Aldo"
        ]
      },
      {
        "name": "Anderson Silva",
        "aliases": [
          "The Spider"
        ]
      },
      {
        "name": "Frankie Edgar",
        "aliases": [
          "Answer"
        ]
      },
      {
        "name": "Dominick Cruz",
        "aliases": [
          "Dominator"
        ]
      },
      {
        "name": "Chris Weidman",
        "aliases": [
          "Weidman"
        ]
      },
      {
        "name": "Demetrious Johnson",
        "aliases": [
          "Mighty Mouse",
          "DJ"
        ]
      },
      {
        "name": "Robbie Lawler",
        "aliases": [
          "Ruthless"
        ]
      },
      {
        "name": "Amanda Nunes",
        "aliases": [
          "The Lioness"
        ]
      },
      {
        "name": "Mauricio Rua",
        "aliases": [
          "Shogun Rua",
          "Shogun"
        ]
      },
      {
        "name": "Joanna Jedrzejczyk",
        "aliases": [
          "Joanna"
        ]
      },
      {
        "name": "Donald Cerrone",
        "aliases": [
          "Cowboy",
          "Cowboy Cerrone"
        ]
      },
      {
        "name": "Daniel Cormier",
        "aliases": [
          "DC"
        ]
      },
      {
        "name": "Forrest Griffin",
        "aliases": [
          "Forrest"
        ]
      },
      {
        "name": "Mark Kerr",
        "aliases": [
          "Smashing Machine"
        ]
      },
      {
        "name": "Vitor Belfort",
        "aliases": [
          "The Phenom"
        ]
      },
      {
        "name": "Wanderlei Silva",
        "aliases": [
          "Axe Murderer"
        ]
      },
      {
        "name": "Jens Pulver",
        "aliases": [
          "Lil Evil"
        ]
      },
      {
        "name": "Kevin Randleman",
        "aliases": [
          "Monster"
        ]
      },
      {
        "name": "Rich Franklin",
        "aliases": [
          "Ace"
        ]
      },
      {
        "name": "Matt Serra",
        "aliases": [
          "Serra"
        ]
      },
      {
        "name": "Kazushi Sakuraba",
        "aliases": [
          "Sakuraba",
          "Gracie Hunter"
        ]
      },
      {
        "name": "Maurice Smith",
        "aliases": [
          "Smith"
        ]
      },
      {
        "name": "Don Frye",
        "aliases": [
          "Frye"
        ]
      },
      {
        "name": "Antonio Rodrigo Nogueira",
        "aliases": [
          "Minotauro",
          "Big Nog",
          "Nogueira"
        ]
      },
      {
        "name": "Bas Rutten",
        "aliases": [
          "El Guapo"
        ]
      },
      {
        "name": "Pat Miletich",
        "aliases": [
          "Croatian Sensation"
        ]
      },
      {
        "name": "Mark Coleman",
        "aliases": [
          "Hammer"
        ]
      },
      {
        "name": "Dan Severn",
        "aliases": [
          "The Beast"
        ]
      },
      "Ken Shamrock"
    ]
  },
  {
    "category": "broadcast",
    "entityKind": "person",
    "collisionGroup": "broadcast",
    "prompts": [
      {
        "prompt": "Name a UFC commentator or analyst fans recognize.",
        "answers": [
          "Joe Rogan",
          "Jon Anik",
          "Daniel Cormier",
          "Michael Bisping",
          "Dominick Cruz",
          "Paul Felder",
          "Laura Sanko",
          "Brendan Fitzgerald"
        ],
        "alsoAcceptedAnswers": [
          "Din Thomas",
          "Anthony Smith",
          "Karyn Bryant",
          "Kenny Florian"
        ]
      },
      {
        "prompt": "Name a voice you associate with UFC broadcasts.",
        "answers": [
          "Joe Rogan",
          "Jon Anik",
          "Daniel Cormier",
          "Michael Bisping",
          "Dominick Cruz",
          "Paul Felder",
          "Laura Sanko",
          "Brendan Fitzgerald"
        ],
        "alsoAcceptedAnswers": [
          "Din Thomas",
          "Anthony Smith",
          "Karyn Bryant",
          "Kenny Florian"
        ]
      },
      {
        "prompt": "Name someone who has called or analyzed UFC fights.",
        "answers": [
          "Jon Anik",
          "Joe Rogan",
          "Daniel Cormier",
          "Michael Bisping",
          "Dominick Cruz",
          "Paul Felder",
          "Laura Sanko",
          "Din Thomas"
        ],
        "alsoAcceptedAnswers": [
          "Brendan Fitzgerald",
          "Anthony Smith",
          "Karyn Bryant",
          "Kenny Florian"
        ]
      },
      {
        "prompt": "Name a UFC broadcaster whose voice is familiar to MMA fans.",
        "answers": [
          "Joe Rogan",
          "Jon Anik",
          "Daniel Cormier",
          "Michael Bisping",
          "Dominick Cruz",
          "Paul Felder",
          "Laura Sanko",
          "Karyn Bryant"
        ],
        "alsoAcceptedAnswers": [
          "Brendan Fitzgerald",
          "Din Thomas",
          "Anthony Smith",
          "Kenny Florian"
        ]
      },
      {
        "prompt": "Name a commentator or analyst strongly connected with UFC coverage.",
        "answers": [
          "Joe Rogan",
          "Jon Anik",
          "Daniel Cormier",
          "Michael Bisping",
          "Dominick Cruz",
          "Paul Felder",
          "Laura Sanko",
          "Brendan Fitzgerald"
        ],
        "alsoAcceptedAnswers": [
          "Din Thomas",
          "Anthony Smith",
          "Karyn Bryant",
          "Kenny Florian"
        ]
      }
    ],
    "answers": [
      {
        "name": "Joe Rogan",
        "aliases": [
          "Rogan"
        ]
      },
      {
        "name": "Jon Anik",
        "aliases": [
          "Anik"
        ]
      },
      {
        "name": "Daniel Cormier",
        "aliases": [
          "DC"
        ]
      },
      {
        "name": "Michael Bisping",
        "aliases": [
          "The Count"
        ]
      },
      {
        "name": "Dominick Cruz",
        "aliases": [
          "Dominator"
        ]
      },
      {
        "name": "Paul Felder",
        "aliases": [
          "Irish Dragon"
        ]
      },
      {
        "name": "Laura Sanko",
        "aliases": [
          "Sanko"
        ]
      },
      {
        "name": "Brendan Fitzgerald",
        "aliases": [
          "Fitz"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Din Thomas",
        "aliases": [
          "Din"
        ]
      },
      {
        "name": "Anthony Smith",
        "aliases": [
          "Lionheart"
        ]
      },
      {
        "name": "Karyn Bryant",
        "aliases": [
          "KB"
        ]
      },
      {
        "name": "Kenny Florian",
        "aliases": [
          "KenFlo"
        ]
      }
    ]
  },
  {
    "category": "officials",
    "entityKind": "person",
    "collisionGroup": "officials",
    "prompts": [
      {
        "prompt": "Name a referee UFC fans recognize.",
        "answers": [
          "Herb Dean",
          "John McCarthy",
          "Marc Goddard",
          "Jason Herzog",
          "Dan Miragliotta",
          "Keith Peterson",
          "Mike Beltran",
          "Mario Yamasaki"
        ],
        "alsoAcceptedAnswers": [
          "Kevin MacDonald",
          "Steve Mazzagatti",
          "Josh Rosenthal",
          "Yves Lavigne",
          "Mark Smith"
        ]
      },
      {
        "prompt": "Name an MMA referee you have seen inside the Octagon.",
        "answers": [
          "Herb Dean",
          "John McCarthy",
          "Marc Goddard",
          "Jason Herzog",
          "Dan Miragliotta",
          "Keith Peterson",
          "Mike Beltran",
          "Kevin MacDonald"
        ],
        "alsoAcceptedAnswers": [
          "Mario Yamasaki",
          "Steve Mazzagatti",
          "Josh Rosenthal",
          "Yves Lavigne",
          "Mark Smith"
        ]
      },
      {
        "prompt": "Name a referee associated with major UFC fights.",
        "answers": [
          "Herb Dean",
          "John McCarthy",
          "Marc Goddard",
          "Jason Herzog",
          "Dan Miragliotta",
          "Keith Peterson",
          "Mike Beltran",
          "Mario Yamasaki"
        ],
        "alsoAcceptedAnswers": [
          "Kevin MacDonald",
          "Steve Mazzagatti",
          "Josh Rosenthal",
          "Yves Lavigne",
          "Mark Smith"
        ]
      },
      {
        "prompt": "Name an official serious UFC fans know by name.",
        "answers": [
          "Herb Dean",
          "John McCarthy",
          "Marc Goddard",
          "Jason Herzog",
          "Dan Miragliotta",
          "Keith Peterson",
          "Mario Yamasaki",
          "Steve Mazzagatti"
        ],
        "alsoAcceptedAnswers": [
          "Mike Beltran",
          "Kevin MacDonald",
          "Josh Rosenthal",
          "Yves Lavigne",
          "Mark Smith"
        ]
      },
      {
        "prompt": "Name a referee whose name UFC fans have heard before a major fight.",
        "answers": [
          "Herb Dean",
          "John McCarthy",
          "Marc Goddard",
          "Jason Herzog",
          "Dan Miragliotta",
          "Keith Peterson",
          "Mike Beltran",
          "Mario Yamasaki"
        ],
        "alsoAcceptedAnswers": [
          "Kevin MacDonald",
          "Steve Mazzagatti",
          "Josh Rosenthal",
          "Yves Lavigne",
          "Mark Smith"
        ]
      }
    ],
    "answers": [
      {
        "name": "Herb Dean",
        "aliases": [
          "Herb"
        ]
      },
      {
        "name": "John McCarthy",
        "aliases": [
          "Big John",
          "Big John McCarthy"
        ]
      },
      {
        "name": "Marc Goddard",
        "aliases": [
          "Goddard"
        ]
      },
      {
        "name": "Jason Herzog",
        "aliases": [
          "Herzog"
        ]
      },
      {
        "name": "Dan Miragliotta",
        "aliases": [
          "Miragliotta"
        ]
      },
      {
        "name": "Keith Peterson",
        "aliases": [
          "No Nonsense"
        ]
      },
      {
        "name": "Mike Beltran",
        "aliases": [
          "Beltran"
        ]
      },
      {
        "name": "Kevin MacDonald",
        "aliases": [
          "MacDonald"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Mario Yamasaki",
        "aliases": [
          "Yamasaki"
        ]
      },
      {
        "name": "Steve Mazzagatti",
        "aliases": [
          "Mazzagatti"
        ]
      },
      {
        "name": "Josh Rosenthal",
        "aliases": [
          "Rosenthal"
        ]
      },
      {
        "name": "Yves Lavigne",
        "aliases": [
          "Lavigne"
        ]
      },
      {
        "name": "Mark Smith",
        "aliases": [
          "Mark Smith ref"
        ]
      }
    ]
  },
  {
    "category": "venues",
    "entityKind": "other",
    "collisionGroup": "venues",
    "prompts": [
      {
        "prompt": "Name a UFC arena, host city, or named event site fans know.",
        "answers": [
          "T-Mobile Arena",
          "Madison Square Garden",
          "MGM Grand Garden Arena",
          "UFC APEX",
          "The O2",
          "Etihad Arena",
          "Fight Island",
          "Honda Center"
        ],
        "alsoAcceptedAnswers": [
          "Abu Dhabi",
          "London",
          "Anaheim",
          "New York City",
          "Las Vegas"
        ]
      },
      {
        "prompt": "Name a UFC arena, host city, or named event site you could picture on a major card.",
        "answers": [
          "T-Mobile Arena",
          "Madison Square Garden",
          "MGM Grand Garden Arena",
          "Etihad Arena",
          "The O2",
          "Honda Center",
          "Las Vegas",
          "New York City"
        ],
        "alsoAcceptedAnswers": [
          "UFC APEX",
          "Fight Island",
          "Abu Dhabi",
          "London",
          "Anaheim"
        ]
      },
      {
        "prompt": "Name an arena, host city, or named event site tied to memorable UFC nights.",
        "answers": [
          "MGM Grand Garden Arena",
          "T-Mobile Arena",
          "Madison Square Garden",
          "Fight Island",
          "The O2",
          "Honda Center",
          "Etihad Arena",
          "UFC APEX"
        ],
        "alsoAcceptedAnswers": [
          "Las Vegas",
          "Abu Dhabi",
          "London",
          "Anaheim",
          "New York City"
        ]
      },
      {
        "prompt": "Name a familiar UFC arena, host city, or named event site.",
        "answers": [
          "T-Mobile Arena",
          "Madison Square Garden",
          "MGM Grand Garden Arena",
          "Las Vegas",
          "Fight Island",
          "Etihad Arena",
          "The O2",
          "UFC APEX"
        ],
        "alsoAcceptedAnswers": [
          "Honda Center",
          "Abu Dhabi",
          "London",
          "Anaheim",
          "New York City"
        ]
      },
      {
        "prompt": "Name an arena, host city, or named event site strongly associated with major UFC events.",
        "answers": [
          "MGM Grand Garden Arena",
          "T-Mobile Arena",
          "UFC APEX",
          "Madison Square Garden",
          "The O2",
          "Honda Center",
          "Las Vegas",
          "Abu Dhabi"
        ],
        "alsoAcceptedAnswers": [
          "Fight Island",
          "Etihad Arena",
          "London",
          "Anaheim",
          "New York City"
        ]
      }
    ],
    "answers": [
      {
        "name": "T-Mobile Arena",
        "aliases": [
          "T-Mobile",
          "T Mobile",
          "T Mobile Arena",
          "TMobile"
        ]
      },
      {
        "name": "Madison Square Garden",
        "aliases": [
          "MSG",
          "Madison Square"
        ]
      },
      {
        "name": "MGM Grand Garden Arena",
        "aliases": [
          "MGM Grand",
          "MGM"
        ]
      },
      {
        "name": "UFC APEX",
        "aliases": [
          "APEX",
          "UFC Apex",
          "Apex"
        ]
      },
      {
        "name": "The O2",
        "aliases": [
          "O2",
          "O2 Arena"
        ]
      },
      {
        "name": "Fight Island",
        "aliases": [
          "Yas Island"
        ]
      },
      {
        "name": "Honda Center",
        "aliases": [
          "Honda"
        ]
      },
      {
        "name": "Etihad Arena",
        "aliases": [
          "Etihad"
        ]
      }
    ],
    "alsoAcceptedAnswers": [
      {
        "name": "Las Vegas",
        "aliases": [
          "Vegas"
        ]
      },
      {
        "name": "Abu Dhabi",
        "aliases": [
          "Abu Dhabi UAE"
        ]
      },
      "London",
      "Anaheim",
      {
        "name": "New York City",
        "aliases": [
          "NYC",
          "New York"
        ]
      }
    ]
  }
] as const);
