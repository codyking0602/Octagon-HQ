...
function member(overrides: Partial<TodayChallengeStandingsEntry>): TodayChallengeStandingsEntry {
  return {
    ...
    gameAverages: { findLeader: 84, wavelength: null, blindResume: 90, blindRank5: 82, keep4Cut4: 87, hitTheNumber: 91, whoAmI: 86, millionaire: 88, sportsFeud: 89 },
...
describe("Daily Challenge championship standings", () => {
  ...
  it("shows Daily Double, Hit the Number, Who Am I, Millionaire, and Sports Feud averages", () => {
    const standings: TodayChallengeStandings = {
    ...
    const millionaire = screen.getByText("Millionaire").closest("span");
    const sportsFeud = screen.getByText("Sports Feud").closest("span");
    expect(dailyDouble).not.toBeNull();
    expect(hitTheNumber).not.toBeNull();
    expect(whoAmI).not.toBeNull();
    expect(millionaire).not.toBeNull();
    expect(sportsFeud).not.toBeNull();
    ...
    expect(within(millionaire as HTMLElement).getByText("88.0")).toBeInTheDocument();
    expect(within(sportsFeud as HTMLElement).getByText("89.0")).toBeInTheDocument();
