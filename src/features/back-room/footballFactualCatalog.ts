export type FootballFactualDomainId = "nfl-qb-career" | "nfl-rb-career" | "cfb-champion-season";

export interface FootballNflQbCareerRow {
  id: string;
  name: string;
  games: number;
  completions: number;
...
export interface FootballNflRbCareerRow {
  id: string;
  name: string;
  games: number;
...
export const footballNflQbCareerRows: readonly FootballNflQbCareerRow[] = [
...
export const footballNflRbCareerRows: readonly FootballNflRbCareerRow[] = [
...
export interface FootballFactualCatalogSubject {
  id: string;
  name: string;
  domainId: FootballFactualDomainId;
}

export const footballFactualCatalogSubjects: readonly FootballFactualCatalogSubject[] = [
  ...footballNflQbCareerRows.map((row) => ({ id: row.id, name: row.name, domainId: "nfl-qb-career" as const })),
  ...footballNflRbCareerRows.map((row) => ({ id: row.id, name: row.name, domainId: "nfl-rb-career" as const })),
  ...footballCfbChampionSeasonRows.map((row) => ({ id: row.id, name: row.name, domainId: "cfb-champion-season" as const })),
];

export const FOOTBALL_FACTUAL_DOMAIN_POOL_SIZE = 25;
export const FOOTBALL_FACTUAL_CATALOG_SUBJECT_COUNT = footballFactualCatalogSubjects.length;

export const footballFactualStatLineSources = [
  {
    ...
    coverage: "25 retired quarterbacks and 25 retired running backs in the canonical Football factual catalog",
  },
  {
    ...
    reviewedOn: "2026-08-22",
    coverage: "25 completed national-championship seasons in the canonical Football factual catalog",
  },
] as const;
